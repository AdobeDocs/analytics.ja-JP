...
| ----------- | ---------- | ------- | ---- |
| **Web SDK を使用したActivity Mapの部分的なサポート** | Web SDK バージョン2.15.0以降では、リンクトラッキングが有効になると、Activity Mapデータの生成が開始されます。 これにより、Web SDK および Analytics で設定された Web SDK とActivity Mapでリンクトラッキングが有効になっている場合に、Web SDK ユーザーがActivity Mapレポートを取得できます。<p>Web SDK を使用したリンクトラッキングを有効にすると、顧客がページ間を移動すると、リンクイベントが送信されることに注意してください。 その結果、追加の課金対象ヒットがAdobeに送信されます。 長期的には、Web SDK で AppMeasurement の機能を反映したActivity Mapを収集する方法に取り組んでいます。 状況によっては、AppMeasurement が後続のページビューで別々にデータを送信するので、サーバーコールの数が減ります。 詳しくは、 [リンクを追跡](https://experienceleague.adobe.com/docs/experience-platform/edge/data-collection/track-links.html?lang=ja) を参照してください。 | 該当なし | 2023年3月31日（PT） |
