...
* のフィルターを適用できない問題を修正しました [!UICONTROL ワークスペース] 「未指定」ディメンションによる列。 （AN-222393）
...
* キュレートされたの問題を修正しました [!UICONTROL 仮想レポートスイート] 仮想レポートスイート内の任意のディメンション/指標で分類できる可能性があります。 （AN-228035）
...
* （[!UICONTROL Customer Journey Analytics]）Customer Journey Analyticsでプロジェクトを共有する際の権限の問題を修正しました。 （AN-226592）
...
* が原因で発生した問題を修正しました _不足しているコンポーネント_ で新規プロジェクトを作成する際にエラーが発生する [!UICONTROL 仮想レポートスイート].（AN-226808）
* が原因で発生した問題を修正しました _不足しているコンポーネント_ 仮想レポートスイートをキュレーション中にエラーが発生しました。 （AN-228257）
...
| 属性：カスタムルックバックウィンドウ | 2020年6月18日 | で任意のアトリビューションモデルを設定できるようになりました [!UICONTROL 帰属] レポート期間の最大 90 日前までのタッチポイントを含めることができます。 これにより、通常、前月（または過去数か月）に発生したインタラクションを考慮することで、レポートの初期に発生するイベントのアトリビューション精度を高めます。Adobe Analytics Foundation、Select、Prime、Premium、Premium Attribution、Premium Complete および Ultimate のお客様が利用できます。 |
...
| 機能 | 説明 |
| -----------| ---------- |
| Adobe Experience Platform Edge Networkに対する Analytics のサポート | Edge Networkを使用して収集したデータをAdobe Analyticsに転送できるようになりました。 |
...
| TLS 1.1 のサポート終了 | 2019年10月3日（PT） | 2020 年 3 月 31 日までに、Adobe Analytics は TLS 1.1 のサポートを削除します。この変更は、最高のセキュリティ標準を維持し、顧客データの安全性を高めるアドビの継続的な取り組みの一環としておこなわれます。 |
| サンノゼ FTP 連携機能の終了 | 2020年7月 | ロンドンおよびシンガポールのお客様に対しては、ロンドンまたはシンガポールとサンノゼのデータセンター間のデータの仲介をサポートしなくなります [ftp.omniture.com](ftp://ftp.omniture.com/).<br/><ul><li>ロンドンのお客様：[ftp3.omniture.com](ftp://ftp3.omniture.com/)</li><li>シンガポールのお客様：[ftp4.omniture.com](ftp://ftp4.omniture.com/)</li></ul> |
...
| Data WarehouseAPI での仮想レポートスイートのサポート | 仮想レポートスイートを、Data WarehouseAPI 経由で使用できるようになりました。 以前は、Data Warehouse UI からしか使用できませんでした。Data WarehouseAPI を使用する際に、仮想レポートスイートを表示してクエリができるようになりました。ただし、これは、仮想レポートスイートに適用されるセグメントがData Warehouseと互換性がある場合に限られます。 |
