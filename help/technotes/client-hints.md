...
クライアントヒントは、2023 年 3 月 2 日（PT）の Analytics デバイス検索プロセスに組み込まれました。 この変更は、AppMeasurementと web SDKの両方の実装に適用されます。
...
+++**ブラウザーのバージョン番号が 999.99 であるのはなぜですか。**

ユーザーは、ブラウザーまたはデバイスのオペレーティングシステムのバージョン番号が無効な場合にテストを実行することがあります。このバージョン番号は、100,000 を超える任意の数になる場合があります。 特定の日に無効なブラウザーまたはデバイスのオペレーティングシステムのバージョン番号が大量に表示されるのを避けるために、100,000 を超えるバージョン番号はバケット化され、バージョン番号 999.99 にピン留めされます。
