...
* **個人データを収集する場合は、プライバシーに関する法律および規制を遵守してください。** カスタム変数を使用すると、アクセスできるほぼすべてのデータを収集できますが、組織のプライバシーポリシーと適用法も考慮する必要があります。
* **組織のプライバシー情報をわかりやすく、見つけやすく提供します。** 役立つ情報には、オプトアウトリンク、閲覧データの使用方法、Adobeのサービスの使用または使用方法が含まれます。
* **お客様に適用される現地の法律と国際法の両方に注意してください。** 自社がグローバルな規模で事業を展開している場合は、いくつかの国際法が適用される可能性があります。
...
| リファラーデータ | データ収集ライブラリは、訪問者がお使いの web サイトに到達すると、デフォルトで参照 URL を収集します。 実装をカスタマイズして、リファラーのクエリ文字列内のデータを収集できます。 これは、キャンペーンや広告のパフォーマンスのトラッキングで一般的です。 | [リファラー](/help/components/dimensions/referrer.md)、[参照ドメイン](/help/components/dimensions/referring-domain.md) |
| 匿名化された訪問者 ID | データ収集ライブラリは、サイトにアクセスするブラウザーごとに訪問者 ID を生成して参照します。 この ID は cookie に保存されます。 データ収集ライブラリで cookie 識別子を設定できない場合、ライブラリでは匿名の訪問者を識別するフォールバックメソッドが使用されます。 このメソッドでは、訪問者の IP アドレスとユーザーエージェント文字列を使用して、関連するヒットを同じ訪問に結び付けます。 お客様の組織で IP の不明化が有効になっている場合、この設定が適用されます。 詳しくは、[Adobe Analytics とブラウザーの Cookie](../cookies/cookies.md) を参照してください。 | [ユニーク訪問者](/help/components/metrics/unique-visitors.md) |
