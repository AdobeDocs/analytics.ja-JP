...
データ管理者が、リクエストの受信と受理について単独で責任を負います。 データ管理者は、データ主体の ID を検証してから、リクエストを実行します。 この責任の一部には、Adobe Analytics に保存されているデータに関連付けられている可能性のあるデータ主体の ID をアドビに通知することが含まれる場合があります。

データプロセッサーとして、アドビは、検証済みリクエストを許容可能な時間内に処理するために、データコントローラーに適切な支援を提供する必要があります。
...
データガバナンスは、Adobe Analytics 内の新しいツールであり、データコントローラーが Analytics データ全体にデータコントロールと分類を適用できるようにします。 この新しいツールにより、アドビのお客様は、データプライバシー関連データのアクセスおよび削除リクエストの処理をカスタマイズできるようになります。 データガバナンスコンソールでは、管理者は、Adobe Analytics に存在する様々なデータ列に適用する設定を定義できます。 これらのラベルが定義されると、アドビは、お客様の希望するラベル設定に従って、すべてのダウンストリームのアクセスリクエストおよび削除リクエストに対応し、処理します。 これらのラベル設定について自社の法務担当者と検討・協議することは、データコントローラーの責任です。

データガバナンスツールには、以下のデータラベルが含まれます。

* **ID データラベル**：直接またはその他のデータと組み合わせて個人を識別できるデータを分類するために使用します。 （なし、I1、I2）。

* **機密データラベル**：適用法の下で機密と定義される可能性のあるデータとしてデータを分類するために使用します。 （なし、S1、S2）現在の Adobe Analytics での機密データの使用は、適用法の下で適切に取得された正確な位置情報データ（一部の管轄地域で機密データと見なされることがあります）を除いて、通常、禁止されています。
...
大規模な訪問者グループ全体にわたって間違って送信されたデータを消去するなど、本来の目的とは異なる用途に Data Privacy API を使用することは避けください。 また、データプライバシー削除リクエストの結果としてヒットが削除（更新または匿名化）された訪問者については、その状態情報がリセットされることも認識しておく必要があります。 訪問者が次回web サイトに戻ったときに、新しい訪問者になります。 すべてのeVar アトリビューションが再度開始され、訪問回数、リファラー、最初のページ訪問などの情報も同様に開始されます。この副作用は、データフィールドをクリアしたい場合には望ましくなく、データプライバシーAPIがこの使用に不適切である理由の1つを示しています。
