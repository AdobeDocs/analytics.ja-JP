...
+ [](home.md)
+ 
+ [アカウントとレポートスイート](landing.md)
+ レポート表示のカスタマイズ{#analysis-workspace}
   + [Google Analytics レポート {#reports}](analysis-workspace/home.md)
   + [Google Analytics ユーザー向け：Analysis Workspace での基本レポートの作成方法](analysis-workspace/new-features-in-analysis-workspace.md)
   + 他の Analytics プラットフォームで使用される一般的な指標{#build-workspace-project}
      + [リアルタイム](analysis-workspace/build-workspace-project/freeform-overview.md)
      + [オーディエンス](analysis-workspace/build-workspace-project/save-projects.md)
      + [獲得](analysis-workspace/build-workspace-project/starter-projects.md)
      + [動作](analysis-workspace/build-workspace-project/multiple-report-suites.md)
      + [コンバージョン数](analysis-workspace/build-workspace-project/fa-shortcut-keys.md)
      + [よくある質問](analysis-workspace/build-workspace-project/color-palettes.md)
      + [プラットフォーム間の処理の違い](analysis-workspace/build-workspace-project/view-density.md)
   + データのサードパーティ製品との比較{#components}
      + [イベントの影響を受けるデータの分析 {#event}](analysis-workspace/components/analysis-workspace-components.md)
      + イベントの影響を受けるデータの分析の概要 {#annotations}
         + [データの急増／急減のトラブルシューティング](analysis-workspace/components/annotations/overview.md)
         + [分析内の特定の日付を除外する](analysis-workspace/components/annotations/create-annotations.md)
         + [日付を前の範囲と比較する](analysis-workspace/components/annotations/manage-annotations.md)
         + [イベントの影響を受けたデータの取得](analysis-workspace/components/annotations/view-annotations.md)
      + イベントについてユーザーに伝える{#dimensions}
         + [インテリジェントデータスムージング](analysis-workspace/components/dimensions/view-dimensions.md)
         + [Cookie {#cookies}](analysis-workspace/components/dimensions/t-breakdown-fa.md)
         + [ブラウザーの cookie](analysis-workspace/components/dimensions/time-parting-dimensions.md)
      + [ブラウザーの cookie 制限の影響を軽減するオプション](analysis-workspace/components/apply-create-metrics.md)
      + リスニングデータの保持期間{#segments}
         + [データの除外](analysis-workspace/components/segments/t-freeform-project-segment.md)
         + [Adobe Analytics が使用する IP とドメイン](analysis-workspace/components/segments/quick-segments.md)
         + [遅延](analysis-workspace/components/segments/ad-hoc-segments.md)
      + 低トラフィック値{#calendar-date-ranges}
         + [Mobile Services の処理ルールの Adobe Analytics への移行](analysis-workspace/components/calendar-date-ranges/calendar.md)
         + [複数通貨のサポート](analysis-workspace/components/calendar-date-ranges/custom-date-ranges.md)
         + [プライバシーの概要](analysis-workspace/components/calendar-date-ranges/time-comparison.md)
   + プログレッシブ Web アプリケーション{#visualizations}
      + [リダイレクトとエイリアス](analysis-workspace/visualizations/freeform-analysis-visualizations.md)
      + [地域データ収集 {#rdc}](analysis-workspace/visualizations/t-sync-visualization.md)
      + 地域データ収集ヘルプ{#freeform-table}
         + [SSL 証明書ライセンス](analysis-workspace/visualizations/freeform-table/freeform-table.md)
         + サポートされる HTTPS 暗号化アルゴリズム{#column-row-settings}
            + [セキュリティリソース](analysis-workspace/visualizations/freeform-table/column-row-settings/column-settings.md)
            + [使用される用語（用語集）](analysis-workspace/visualizations/freeform-table/column-row-settings/table-settings.md)
            + [タイムスタンプオプション](analysis-workspace/visualizations/freeform-table/column-row-settings/manual-vs-dynamic-rows.md)
         + [ログインのトラブルシューティング](analysis-workspace/visualizations/freeform-table/pagination-filtering-sorting.md)
         + [ログアウトのトラブルシューティング](analysis-workspace/visualizations/freeform-table/workspace-totals.md)
      + 未指定、その他および不明{#cohort-table}
         + [訪問者の移行](analysis-workspace/visualizations/cohort-table/cohort-analysis.md)
