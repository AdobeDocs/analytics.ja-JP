...
| 4 | fid（フォールバックcookie） | 訪問者のブラウザーが cookie（ファーストパーティ）を受け入れる場合 |
...
このサービスに関する製品ドキュメントについては「[ID サービス](https://marketing.adobe.com/resources/help/en_US/mcvid/)」を参照してください。
...
この場合、[動的変数](../implement/vars/page-vars/dynamic-variables.md)を使用すると、ヘッダーから特定の値を抽出できます。
