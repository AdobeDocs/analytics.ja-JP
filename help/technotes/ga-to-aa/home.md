...
Adobe Analyticsでは、Adobe CX Enterpriseにログインした後に基本的なレポートを取得する主な方法は、**Analysis Workspace**&#x200B;を使用することです。 左側のメニューでは、コンポーネントをドラッグ＆ドロップして独自のレポートを作成できます。 レポートに関する正確なニーズにより多くの自由度を持って対応できる。 このレイアウトは、Google Analytics のダッシュボードとカスタムレポートに類似しています。
