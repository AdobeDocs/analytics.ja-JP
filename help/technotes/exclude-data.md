...
データの除外は、通常、組織のテスト作業によって本番データが汚染されるのを防いだり、不要なデータによってレポートが誤って水増しされるのを防いだりするために使用されます。 ユースケースに応じて、次の方法を使用して Adobe Analytics からデータを除外します。

## データ収集後のデータの除外

次に示す方法は、Adobe データ収集サーバーがイメージリクエストを受信した後に、Analytics のレポートからデータを除外するための手段です。 これらの方法を使用して除外されたデータは、引き続き請求対象のサーバー呼び出しにカウントされます。
...
特定のヒットが Analytics データ収集サーバーに到達するのを防ぐには、[`abort`](/help/implement/vars/config-vars/abort.md) 変数を使用します。 このフラグは、イメージリクエストが送信されるのを防ぎます。 中止されたイメージリクエストはアドビのデータ収集サーバーに到達しないため、請求対象となるサーバーコール数は増加しません。
