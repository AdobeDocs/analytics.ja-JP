...
* **マニフェスト**:フィード間隔に対してデータが収集されない場合に、Adobeが [マニフェスト](c-df-contents/datafeeds-contents.md#feed-manifest) ファイルを宛先に配信するかどうかを指定します。マニフェストファイルを選択した場合、データが収集されない場合、次のようなマニフェストファイルを受け取ります。
