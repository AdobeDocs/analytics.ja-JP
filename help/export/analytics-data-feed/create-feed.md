...
1. 「[!UICONTROL **詳細**]」セクションで、次のフィールドに入力します。
   ...
   | [!UICONTROL **タグ**] | 任意のタグをデータフィードに適用して分類を容易にします。 「[ フィルター」の説明に従ってタグでフィルターを実行し、[ データフィードの管理](/help/export/analytics-data-feed/df-manage-feeds.md)でデータフィードのリスト ](/help/export/analytics-data-feed/df-manage-feeds.md#filter-and-search-the-list-of-data-feeds)を検索できます。 |
