...
1. 「[!UICONTROL **詳細**]」セクションで、次のフィールドに入力します。
   ...
   | [!UICONTROL **タグ**] | 任意のタグをデータフィードに適用して分類を容易にします。 「[&#128279;](/help/export/analytics-data-feed/df-manage-feeds.md#filter-and-search-the-list-of-data-feeds)&#x200B; フィルター」の説明に従ってタグでフィルターを実行し、[&#x200B; データフィードの管理](/help/export/analytics-data-feed/df-manage-feeds.md)でデータフィードのリスト &#x200B;を検索できます。 |
...
1. 「[!UICONTROL **データ形式**]」セクションで、次の情報を指定します。
   ...
   | **到着遅延のヒットを許可** | 履歴データは、データフィードジョブが特定の時間または日付の処理を完了した後に（タイムスタンプ付きのヒットやデータソースを通じてなど）到着する場合があります。<p>設定したレポート頻度（通常は日単位または時間単位）内でデータフィードジョブがデータ処理を完了した後に到着したデータも含める場合は、このオプションを選択します。 このオプションを有効にすると、データフィードはデータを処理するたびに、到着が遅れたヒットを確認し、送信される次のデータフィードファイルにまとめて含めます。</p><p>詳しくは、[着信遅延](/help/export/analytics-data-feed/c-df-contents/late-arriving-hits.md)を参照してください。</p> |
...
1. [!UICONTROL **データ構造**] セクションの&#x200B;**[!UICONTROL レポートスイート]** フィールドで、書き出すデータを含むソースレポートスイートを選択します。 <p>レポートスイートを選択する際には、次の点を考慮してください。</p> <ul><li>同じレポートスイートに対して複数のデータフィードを作成する場合、各データフィードに異なる列定義が必要です。</li><li>データフィードをサポートするのはソースレポートスイートのみです。仮想レポートスイートはサポートされていません。</li><li>使用可能な列のリストは、選択したレポートスイートが属するログイン会社によって異なります。 レポートスイートを変更すると、使用可能な列のリストが変更される可能性があります。 </li></ul>
...
1. 「[!UICONTROL **スケジュール**]」セクションで、次の情報を指定します。
   ...
   | フィールド | 関数 |
   |---------|----------|
   | [!UICONTROL **頻度**] | データフィードを送信する頻度を選択します。 使用可能なオプションは、レポートスイート設定に基づいて動的に入力されます。 <p>一般的に、次のオプションを使用できます。</p><ul><li>**毎日**: フィードには、レポートスイートのタイムゾーンの午前0時から午前0時までの1日分のデータが含まれます。 このオプションは、バックフィルまたは履歴データ、または継続的なフィードに使用します。</li><li>**時間単位**: フィードには、1時間のデータが含まれます。 このオプションは、フィードを継続する場合に使用します。</li></ul><p>15分の書き出し頻度は可能ですが、デフォルトでは使用できません。 このオプションをお使いの環境で使用できるようにするには、まずアドビカスタマーケアに問い合わせて、15 分の書き出しをサポートするようにレポートスイートが設定されていることをリクエストする必要があります。</p> |
   | [!UICONTROL **処理遅延**] | データフィードファイルを処理する前に、一定時間待つかどうかを選択します。 遅延は、モバイル実装に、オフラインデバイスがオンラインになり、データを送信する機会を与えるのに役立ちます。 また、以前に処理されたファイルを管理する際に、組織のサーバーサイドのプロセスに対応するためにも使用できます。 ほとんどの場合、遅延は必要ありません。 フィードを最大8時間（480分）遅らせることも、カスタム時間（9,999分、約1週間）を選択した場合はさらに長くすることもできます。 |
   | [!UICONTROL **連続フィード**] | このオプションを選択すると、終了日が削除され、フィードを無期限に実行できるようになります。 フィードが履歴データの処理を完了すると、フィードはデータが特定の時間または日に収集を完了するのを待ちます。 現在の時間または日が終了すると、指定された遅延の後に処理が開始されます。 |
   | [!UICONTROL **開始日**] | データフィードを開始する日付を指定します。 履歴データのデータフィードの処理をすぐに開始するには、データが収集されている過去の任意の日付にこの日付を設定します。 開始日は、レポートスイートのタイムゾーンに基づきます。 |
...
1. 「[!UICONTROL **宛先**]」セクションで、データを送信する宛先を設定します。
   ...
   >
   >* クラウドアカウントは Adobe Analytics ユーザーアカウントに関連付けられています。 他のユーザーは、組織内のすべてのユーザーが利用できるようにしない限り、設定したクラウドアカウントを使用または表示できません。
   >
   ...
   | フィールド | 関数 |
   |---------|----------|
   | [!UICONTROL **アカウント**] | 次のいずれかの操作を行います。<ul><li>**既存のアカウントを使用：** 「**[!UICONTROL アカウント]**」フィールドの横にあるドロップダウンメニューを選択します。 または、アカウント名の入力を開始し、ドロップダウンメニューから選択します。 <p>アカウントは、設定した場合、または自分が所属する組織と共有されている場合にのみ利用できます。</p></li><li>**新しいアカウントを作成します：** 「**[!UICONTROL アカウント]**」フィールドの下にある「**[!UICONTROL 新しい]**&#x200B;を追加」を選択します。 アカウントの設定方法について詳しくは、[&#x200B; クラウドの読み込みと書き出しのアカウントの設定](/help/components/locations/configure-import-accounts.md)の[場所アカウントの設定](/help/components/locations/configure-import-accounts.md#configure-a-location-account)を参照してください。</li></ul> |
   | [!UICONTROL **場所**] | 次のいずれかの操作を行います。<ul><li>**既存の場所を使用：** 「**[!UICONTROL 場所]**」フィールドの横にあるドロップダウンメニューを選択します。 または、場所の名前を入力し、ドロップダウンメニューから選択します。</li><li>**新しい場所を作成：**&#x200B;選択&#x200B;**[!UICONTROL 場所]** フィールドの下に&#x200B;**[!UICONTROL 新しい]**&#x200B;を追加します。 場所の設定方法について詳しくは、[&#x200B; クラウドの読み込みと書き出しの場所の設定](/help/components/locations/configure-import-locations.md)の[場所の設定](/help/components/locations/configure-import-locations.md#configure-a-location)を参照してください。</li></ul> |
