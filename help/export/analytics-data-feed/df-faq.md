...
標準時間（STD）から夏時間（DST）への移行時（夏時間への時計調整時）は、ユーザーの取得するファイル数が 23 個になります。DST への移行でスキップされた時間分は省略されます。例えば、移行が午前 2 時におこなわれる場合、午前 1 時のファイルの取得に続いて、:00 時 :00 のファイルを取得することになります。 :00 のファイルは存在しません。これは、STD の 2:00 では DST の 3:00 になるからです。

DST から STD への移行時（冬時間への時計調整時）は、ユーザーの取得するファイル数は 24 個になります。ただし、実際には移行の時間に 2 時間分のデータが含まれることになります。例えば、トランジションが午前 2:00 に発生した場合、1:00 のファイルは 1 時間遅延しますが、2 時間のデータが含まれています。 含まれるデータは、DST が 1:00 から STD が 2:00 （DST が 3:00）までのものです。 次のファイルは STD:002 から始まります。
