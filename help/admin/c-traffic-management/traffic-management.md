---
description: トラフィック管理ページでは、予想されるトラフィック量の変化を指定できます。これらの設定により、適切なリソースを割り当て、トラフィックをタイムリーに追跡して処理することができます。
title: トラフィックの管理
feature: Traffic Management
exl-id: 7a7a36f2-3241-47f5-9c0f-0d62e51104e1
source-git-commit: 72bd67179e003b70233d863d34153fec77548256
workflow-type: tm+mt
source-wordcount: '91'
ht-degree: 86%

---

# トラフィックの管理

トラフィック管理ページでは、予想されるトラフィック量の変化を指定できます。これらの設定により、適切なリソースを割り当て、トラフィックをタイムリーに追跡して処理することができます。

[!UICONTROL トラフィック管理]にアクセスするには：

1. クリック **[!UICONTROL Analytics]** > **[!UICONTROL 管理者]** > **[!UICONTROL すべての管理者]** > **[!UICONTROL レポートスイート]**」をクリックし、レポートスイートを選択します。

1. **[!UICONTROL 設定の編集]**／**[!UICONTROL トラフィック管理]**／*`<selection>`*&#x200B;の順にクリックします。
