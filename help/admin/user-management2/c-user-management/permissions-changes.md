...
>[!NOTE]使用可能な権限の組み合わせが多いので、権限の組み合わせごとに使用できる API メソッドをすべて説明するドキュメントを用意することはできません。一般に、Webサービスへのアクセスを許可された管理者以外のユーザーは、APIメソッドへの読み取りアクセス権のみを持ちます。 メソッドに対する書き込みアクセス権はありません。

APIとインターフェイスは同じ権限設定システムを使用するので、特定の非管理者がインターフェイスの管理者(Adobe Admin Console)から付与された権限は、APIでのユーザーと同じ権限になります。
...
<table id="table_D1DB0DE37752450BBCCA44DB760BB505"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> <b>管理者アクセス：</b> 管理者には、定義済みのグループは不要になりました。 管理者は、すべての項目（ツール、指標、ディメンション）に加え、Webサービス、Report Builder、アクティビティマップおよびアドホック分析にアクセスできるようになりました。 </p> <p>今後、グループの目的は、管理者以外のユーザーにアクセスを許可または制限することです。 </p> <p> <b>カスタムグループ：</b> カスタムグループが、事前定義されたグループを置き換えました。 既存の定義済みグループは、同じグループ名を使用してカスタムグループに移行されます。 作成したカスタムグループ（その設定を含む）はすべて保持されます。 ただし、設定の場所が移動されます。 例えば、（管理コンソールのカスタマイズの）会社設定がAnalyticsツールのカスタマイズに <a href="/help/admin/user-management2/c-customize-report-access/groups-analytics-tools.md"> なりました</a>。 </p> <p> 「<span class="term">すべてのレポートアクセス</span>」に属するユーザーは、次のアクセス権を持つカスタムグループに移行されました。 </p> 
    <ul id="ul_696A9243F5FD4AF187352C2F4B1CFDC2"> 
     ...
     <li id="li_AFFB58C7FB644AC8A85E2D76BA7D51F5">分析ワークスペースのアクセス権限 </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>権限をカスタマイズして、（指標に加えて）ディメンションへのアクセスを含めたり除外したりできます。 </p> 
    <ul id="ul_DA5A54223673474E9151AF979DA50659"> 
     <li id="li_C3E82F7BC07A4F2F83A85D3D511292CC"> <p>カスタムグループ内の現在のすべてのディメンションと指標は、新しいディメンションに自動的に移行されました。カテゴリ 既存のグループで指標が有効になっている場合、新しく権限を付与可能なすべてのディメンション（eVarおよびコンテンツに応じたディメンション）と指標がデフォルトで提供されます。 </p> </li> 
...
新しいおよび予定されている更新と、それらが管理環境に及ぼす影響に関する重要な新情報です。
...
<table id="table_1E93F45C66E841E6882FB602509F30A3"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1">2016年7月のリリースでは、どの権限が変 <b>更されました</b> か。 </td> 
  ...
  <tr> 
   <td colname="col1"> <p>ユーザーを移行するために必要なことはありますか。 </p> </td> 
   <td colname="col2"> <p>いいえ、すべての権限の移行は透過的に行われます。 </p> <p> 
     <ul id="ul_654F85286EC04416B3E0BA725EBE10AD"> 
      <li id="li_8050B8941F794103B82A0ADF0930D216">カスタムグループ内の現在のすべてのトラフィックレポートは、新しいディメンションレポートに自動的に移行されますカテゴリ。 </li> 
      <li id="li_B97079DB29A346B98D066F11AB7F94AF">カスタムグループで既に有効になっている指標がある場合、新しく権限を付与できるすべてのディメンション（eVarおよびソリューション変数）が自動的に割り当てられます。 </li> 
      <li id="li_F1219EF490DA473BA15F2B215F2995AE"> 少なくとも1つの指標を持つカスタムグループには、新たに利用可能になったトラフィックディメンション <b></b> （旧称トラフィックレポート）を除く、すべてのeVarおよびその他のコンテンツに応じたディメンションへのアクセス権が自動的に付与されます。 </li> 
      ...
      <li id="li_2FCD9254FC3C4FD7871EEF9453E5CE1E">指標を含むすべてのカスタムグループには、新しい指標として追加されたすべてのAnalyticsソリューションイベントが含まれます。 </li> 
      <li id="li_34C4560769B64F28A4E83BAE71065DCC">全レポートアクセスに属していたすべてのユーザーは、新しいカスタムグループに追加されます。すべてのレポートアクセスは存在しなくなります。 </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>何が変わらないの？ </p> </td> 
   <td colname="col2"> <p>訪問者属性は、引き続き権限がありません。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
>[!NOTE]*`migrated user`* および *`Experience Cloud user`* は、Experience Cloud に参加するための電子メールの招待状を承認したユーザーを指します。電子メールの招待状が承認されなかった場合、ユーザーは、依然として Analytics ユーザーで、Admin Console で管理できません。(ただし、移行でEnterprise IDまたはFederated IDを使用し [ている場合は例外です](https://helpx.adobe.com/jp/enterprise/using/set-up-identity.html)。 この場合、管理者がユーザーをユーザーごとに移行する際に、ユーザーが移行されます)。

<table id="table_B68FD00FC5D24823A86BB69558C0327C"> 
 <thead> 
  <tr> 
   <th colname="col1" class="entry"> タスク </th> 
   <th colname="col2" class="entry"> 移行しないログイン会社 </th> 
   <th colname="col3" class="entry"> 現在の移行会社 </th> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col4"> <p> <a href="https://adminconsole.adobe.com/enterprise/">Admin Console</a> </p> <p> 管理ツール — 編集は、APIキー管理と、アセットの削除/転送に制限されます。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> Analyticsへのログイン </td> 
   <td colname="col2"> <p> <b>Experience Cloud: </b> marketing.adobe.com <span class="filepath"></span>. Experience Cloudユーザーのみが利用できます。 </p> <p> <b>Analytics（レガシー）：</b><span class="filepath">sc.omniture.com</span>。Analyticsのみのユーザー、およびAnalyticsの資格情報を持つExperience Cloudユーザーの場合 </p> </td> 
   <td colname="col3"> <p> <span class="filepath"> marketing.adobe.com</span> - Experience Cloudユーザーのみ利用できます。 </p> <p> <span class="filepath">sc.omniture.com</span> - Analytics のみのユーザー、および Analytics 資格情報を持つ Experience Cloud ユーザー。 </p> <p>移行中、管理者は、特定のユーザーの <span class="filepath">omniture.com</span> ログイン機能をオフにできます。 </p> </td> 
