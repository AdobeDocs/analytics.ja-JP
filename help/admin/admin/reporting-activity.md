...
レポートアクティビティマネージャーでは、組織内の各レポートスイートのレポート容量を確認できます。 これにより、管理者はレポートの消費状況を詳細に把握し、ピーク時のレポート作成時に容量の問題を簡単に診断および修正できます。
...
| **[!UICONTROL ステータス]** | 次の 4 つのステータスインジケーターが使用可能です。 <ul><li>**赤 — [!UICONTROL 容量で]**:レポートスイートは、レポートの容量に関して最大限に使用できます。 (95% - 100%) </li><li>**黄色 — [!UICONTROL 容量に近づく]**:このレポートスイートは、最大容量に達する恐れがあります。 (90% - 94%)</li><li>**緑 — [!UICONTROL すべて良好]**:レポートには十分な容量があります。 (0% - 90%)</li><li>**グレー — [!UICONTROL ステータス保留中]**:?</li></ul> |
...
| 数値の概要 | 説明 |
| --- | --- |
| ユーザー | 現在このレポートスイートにレポートリクエストを送信しているユーザーの数。 |
| プロジェクト | Workspace プロジェクト、Report Builderワークブックなど。 |
| クエリ | 現在実行中のクエリの数。 |
| 平均待機時間 | 実行中のすべてのクエリの平均待機時間です。 |
...
| 実行時間 | クエリが実行されている期間。 |
| 待機時間 | クエリが処理されるまでに待機している時間。 通常は、十分な容量がある場合は「0」です。 |
| 開始時間 | クエリが処理を開始したとき（管理者のローカル時間）。 |
| アプリケーション | レポートアクティビティマネージャーでサポートされるアプリケーションは次のとおりです。 <ul><li>Analysis Workspace UI</li><li>Workspace スケジュール済みプロジェクト</li><li>Report Builder</li><li>ビルダー UI:セグメント、計算指標、注釈、オーディエンスなど。</li><li>1.4 または 2.0 API からの API 呼び出し（5 つの同時リクエスト）</li><li>インテリジェントアラート</li></ul> |
| ユーザー | クエリを開始したユーザー。 |
| プロジェクト | Workspace プロジェクト、Report Builderワークブックなど。 |
| 月の境界 | リクエストが横切る月の境界の数。 これにより、リクエストがより複雑になります。 |
| 列 | リクエストの複雑さを測定するための Workspace の指標と分類の数。 |
| セグメント | このリクエストに適用されるセグメントの数。 これにより、リクエストがより複雑になります。 |
| ステータス | 次の 4 つのステータスインジケーターが使用可能です。 <ul><li>**赤 — [!UICONTROL 容量で]**:レポートスイートは、レポートの容量に関して最大限に使用できます。 （95%以上）</li><li>**黄色 — [!UICONTROL 容量に近づく]**:このレポートスイートでは、最大容量 (90% ～ 95%) に達する恐れがあります。</li><li>**緑 — [!UICONTROL すべて良好]**:レポートには十分な容量があります。</li><li>**[!UICONTROL ステータス保留中]**:ステータスが利用できません。</li></ul> |
