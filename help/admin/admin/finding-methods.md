---
description: 検索方法ページでは、各種の検索方法レポートがサイト上のコンバージョン成功イベントのクレジットを受け取る方法を示します。 例えば、検索エンジンが購入を行った訪問者をサイトで参照する場合、検索方法は照会のクレジットを検索エンジンが受け取る方法を指定します。
title: 検索方法
topic: Admin tools
...
検索方法ページでは、各種の検索方法レポートがサイト上のコンバージョン成功イベントのクレジットを受け取る方法を示します。 例えば、検索エンジンが購入を行った訪問者をサイトで参照する場合、検索方法は照会のクレジットを検索エンジンが受け取る方法を指定します。
...
<table id="table_8ABC1C9BD63F419082E4C4C69E401526"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> 照会のクレジットを適用する方法を指定します。 次の配分オプションがサポートされています。 <p> <span class="uicontrol">最新（最後）：</span>最後のリファラーに全クレジットを与えます。 </p> <p> <span class="uicontrol">元の値：</span>最初のリファラーに全クレジットを与えます。 </p> <p> <span class="uicontrol">線形：</span>すべてのリファラー間で等しく配分されます。 </p> </td> 
  </tr> 
  <tr> 
...
>[!NOTE]すべての検索方法はその訪問が終了すると有効期限が切れます。「有効期限」を別のイベント（「チェックアウト」など）に設定した場合、検索方法は訪問中にチェックアウトが発生すると有効期限が切れます。 訪問中にチェックアウトが発生しなかった場合、検索方法は訪問が終了しても有効期限が切れます。
