...
4. 目的のレポートスイートを選択し、設定の編集 [!UICONTROL /コンバージョン] / [!UICONTROL 成功イベ] ントに移動します 。
5. 目的のイベントを見つけ、「個別イベントの記録 [!UICONTROL 」ドロップダウンを「訪問ご] とに1回記録 [!UICONTROL 」または「イベントID] を使用」に変更します 。
...
| 個別イベントの記録 | **訪問ごとに1回記録**:指定されたイベントを訪問者のセッションに結び付けます。 同じ訪問中の特定のイベントに対する後続のカウントは無視されます。 このタイプのイベントシリアル化では、実装の変更は必要ありません。<br>**イベントIDを使用&#x200B;**:渡されたイベントをカスタムIDに結び付けます。 同じイベントIDを持つ特定のイベントに対する後続のカウントは無視されます。 このタイプのイベントシリアル化では、値の重複を除外するために、ヒットのカスタムIDが必要です。 『導入ユ[ーザガイド](../../../implement/vars/page-vars/events/event-serialization.md)』の「イベントIDシリアル化」を参照してください。 |
