...
* 特定のリクエストをキャンセルしたり、特定のユーザーからのすべてのリクエストをキャンセルしたり、特定のプロジェクトに関連するすべてのリクエストをキャンセルしたりできます。
  ...
  リクエストをキャンセルすると、アクションは [ログ](/help/admin/admin/logs.md). The [!UICONTROL **イベントタイプ**] 列は次のように表示されます。 [!UICONTROL **管理アクション**]&#x200B;に設定されている場合、キャンセルの説明は [!UICONTROL **イベント**] 列。

* リクエストをキャンセルする場合、指定した期間に対して以降のリクエストを制限することもできます。

  後続のリクエストを制限すると、アクションは [ログ](/help/admin/admin/logs.md). The [!UICONTROL **イベントタイプ**] 列は次のように表示されます。 [!UICONTROL **管理アクション**]&#x200B;に設定されている場合、制限の説明は [!UICONTROL **イベント**] 列。
...
1. （オプション）指定した期間の今後のリクエストを制限する手順は、次のとおりです。
   ...
   1. 次のオプションから選択します。
      ...
      | オプション | 関数 |
      |---------|----------|
      | [!UICONTROL **ユーザーおよびプロジェクト**] | 選択したユーザーは、関連付けられたプロジェクトのレポートリクエストが一時的に制限されます。 |
...
1. （オプション）指定した期間の今後のリクエストを制限する手順は、次のとおりです。
   ...
   1. 次のオプションから選択します。
      ...
      | オプション | 関数 |
      |---------|----------|
      | [!UICONTROL **ユーザーおよびプロジェクト**] | 選択したプロジェクトは、関連付けられたユーザーによるレポートリクエストが一時的に制限されます。 |
