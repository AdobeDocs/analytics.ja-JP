...
データプライバシーアクセス要求用に返されたヒットレベル CSV ファイルは、これらのフィールドの値を unix タイムスタンプから形式の日付/時刻フィールドに変換します `YYYY-MM-DD HH:MM:SS` ( 例： `2018-05-01 13:49:22`) をクリックします。 サマリーHTMLファイルでは、これらのタイムスタンプ値は、日付のみを含むように切り捨てられます。 `YYYY-MM-DD`を使用して、これらのフィールドで発生する一意の値の数を減らします。
