...
* eVarをリセットすると、有効期限切れにすることができます。これは変数を再利用する場合に便利です。

例えば、eVarの有効期限を30日から90日に変更した場合、収集されたeVar値は、新しい有効期限セット（この場合は90日）の間、引き続き保持されます。 現在の有効期限の設定と、収集されたeVar値の最後の設定タイムスタンプを調べて有効期限を判断します。 「 **[!UICONTROL リセット]** 」オプションのみが値の有効期限が切れ、即座に切れます。

別の例： 5月にeVarを使用して内部プロモーションと21日後に期限切れになり、6月に内部検索キーワードを取り込む場合は、6月1日に変数の有効期限を強制的に切るかリセットする必要があります。 これにより、内部プロモーションの値を 6 月のレポートから除外できます。
