...
Adobe は、高トラフィックのイベントが成功するよう、クライアントとの提携を試みます。トラフィックスパイクのスケジュールを設定することは、パートナープロセスの出発点となります。「スパイクのスケジュール」セクションでは、一時的なトラフィックスパイクをアドビに警告し、その処理に適切なリソースに割り当てることができるようにします。過去のサーバーコールを推定して、スケジュールする必要のあるトラフィックスパイクのサイズをより深く把握できます。
...
1. （オプション）過去のサーバーコールを推定して、スケジュールする必要があるトラフィックスパイクのサイズをより深く把握できます。

   例えば、特定の期間内で昨年の日別サーバーコール数の平均に加えて、今年のサーバーコール数の予想増加量を取得できます。 その後、この増倍率に基づいて、トラフィックスパイクをスケジュールできます。

   1. 内 **[!UICONTROL 過去のサーバーコール]** 「 」領域で、選択したレポートスイートの開始日と終了日を選択します。

      金額： [!UICONTROL **ピーク日**], [!UICONTROL **ピーク日のサーバーコール**]、および [!UICONTROL **サーバーコールの日平均**] が生成されます。

   1. 乗算の係数の値を入力し、「 **[!UICONTROL クリックして乗算と設定]**.

      各レポートスイートの各列の値が乗算されます。
1. 内 [!UICONTROL **スパイクパラメータの設定**] 領域、 **[!UICONTROL スパイク開始日]** 「 」フィールドで、トラフィックスパイクを開始する日付を指定します。
1.  「**[!UICONTROL スパイク終了日]**」フィールドでトラフィックスパイクを終了する日付を指定します。
1. 内 **[!UICONTROL ピーク日のサーバーコール]** 「 」フィールドで、トラフィックスパイク期間中の 1 日あたりの合計ピークページビュー数を指定します。
1. 内 **[!UICONTROL ピーク時間のサーバーコール]** 「 」フィールドで、トラフィックスパイク期間中の 1 時間あたりの合計ピークページビュー数を指定します。
