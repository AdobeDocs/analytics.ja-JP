...
ただし、通常の設定では、実稼動レポートスイートに要求を適用する前にこれらのテストレポートスイートで GPDR 要求処理をテストすることはできません。これは、データプライバシー要求が Experience Cloud 組織内のすべてのレポートスイート（多くの場合、会社のすべてのレポートスイート）に自動的に適用されるからです。
