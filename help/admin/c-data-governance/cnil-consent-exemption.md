...
* 13 か月間の Cookie 制限。   `cookieLifetime` 変数を使用して、Analytics の Cookie の有効期限を上書きできます。  AnalyticsやECIDを含むExperience Cloudcookieは、各訪問でcookieの有効期限を延長します。  cookieの無期限を静的に設定するには、次のいずれかを実行します。(1) cookieを削除する日付を設定するカスタムコードを記述するか、(2) cookieのリセット日を制御するためにCMPを使用します。   [cookieLifetimeおよび](https://experienceleague.adobe.com/docs/analytics/implementation/vars/config-vars/cookielifetime.html?lang=ja)  [Experience Cloudcookie](https://experienceleague.adobe.com/docs/core-services/interface/ec-cookies/cookies-privacy.html?lang=en#ec-cookies)
