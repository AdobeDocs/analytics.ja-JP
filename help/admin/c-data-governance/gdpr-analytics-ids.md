...
どのラベルをどの変数/フィールドに適用するかを判断する前に、Analyticsデータで取得するIDを理解し、データプライバシーリクエストに使用するラベルを決定する必要があります。 データプライバシーは、IDと見なされる対象の範囲を拡張します。 ID は、個人を直接的に特定できるもの（ID ラベル：I1）と、間接的に特定できるもの（ID ラベル：I2）の 2 種類に大別されます。
...
次の表を使用して、データプライバシーリクエストをAnalyticsに送信する際に使用するIDのタイプを決定します。 この情報を知れば、変数に使用すべき他のラベルを簡単に判別できます。
  ...
  <tr> 
   ...
   <td colname="col2"> <p><a href="https://marketing.adobe.com/resources/help/en_US/sc/implement/props_eVars.html" format="html" scope="external">カスタムトラフィック変数（prop）またはカスタムコンバージョン変数（eVar）</a>に ID を挿入しているお客様もいます。最も一般的なのは CRM ID ですが、それ以外にも、電子メールアドレス、ユーザーログイン名、顧客のロイヤルティ番号、またはこれらの値のハッシュなどがあります。 </p> 
    <ul id="ul_0B9492CF786046BB97E31CCF83A85FEA"> 
     <li id="li_D35B61CC6A8B485A8E09358A46D3F598">データのプライバシーリクエストにこれらのIDの1つを使用する場合は、そのIDを含むフィールドにID-PERSONラベルを指定する必要があります。 </li> 
    ...
    </ul> <p> データプライバシーリクエストのデータサブジェクトの識別に使用するIDを含むカスタム変数が、レポートスイートに1つ以上、2つ以上存在することはまれです。 通常、ID-PERSON または ID-DEVICE ラベルもあるのは、そのうちの 1 つまたは 2 つのみです。 </p> </td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>propは常に大文字と小文字が区別されません。 eVar もデフォルトでは大文字と小文字が区別されませんが、アドビカスタマーケア経由で区別するように変更できます。IDを含む大文字と小文字の区別があるeVarの場合は、データプライバシーリクエストを送信する際に、リクエストで使用される大文字と小文字が一致するように、適切な大文字と小文字を区別する必要があります。
...
他のラベルが設定されるフィールドはほとんどありませんが、多くの場合は大部分のフィールドに ACC ラベルが設定されます。適切なアクセスラベルは、データプライバシーリクエストに使用しているIDによって異なります。
