...
* DEL- ラベルを設定した変数は[匿名化](/help/admin/c-data-governance/data-labeling/gdpr-labels.md#data-governance-labels)されます（ID が「Data Privacy-」というプレフィックスが付いたランダムの文字列に置き換えられます）。要求で使用された ID によって識別されるすべてのヒットの元の値が、匿名化された同一の値に置き換えられます。このフィールドの元の値がこの ID のいずれかだった場合は、レポートの指標は変わりません。
