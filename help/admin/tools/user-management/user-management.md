...
| タスクまたはリソース | 説明 |
| --- | --- |
| Analytics ユーザー ID の Adobe Admin Console への移行 | Adobeは、Analytics管理者がユーザーIDをAdobe Admin Consoleに移行するのを支援します。 この対応は順次行われます。 ユーザーを移行する順番が来たら、AdobeからAnalytics管理者に指示を電子メールで通知します。 このタスクを簡素化するために、 [Analytics ユーザー管理](/help/admin/tools/user-management/user-migration/c-migration-tool.md) で移行ツールを使用できます。<p>**重要**：ユーザーの移行当日に、以前の権限グループが自動的に Adobe Admin Console にコピーされます。 Analytics 管理ツールで新規ユーザーを招待したり、新規グループを作成したりできなくなります。 移行の準備方法と影響を受ける管理機能については、「Adobe Admin Console への Analytics ユーザーの移行」の FAQ とヘルプを参照してください。 |
| Adobe Admin Console の起動 | ユーザーアカウントが移行されたら、すべてのソリューションのユーザーおよび製品を Adobe Admin Console で管理できます。 `https://adminconsole.adobe.com/enterprise/` に移動します。 [Manage CX Enterprise ユーザーと製品](https://experienceleague.adobe.com/docs/core-services/interface/administration/admin-getting-started.html?lang=ja)も参照してください。 |
