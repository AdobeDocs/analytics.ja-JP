...
さらに、すばやく変形するボットに対応するために、アドビでは、適切かつ定期的に組み合わせることで、データ品質に悪影響を与えるボットを排除するのに役立つ、強力な機能をいくつか提供しています。 Experience Cloud ID サービス、セグメンテーション、Data Warehouse、顧客属性、およびバーチャルレポートスイートの機能です。 以下に、これらのツールを使う方法の概要を示します。
...
この ID をデータ要素経由で取得する方法を次に示します。 CX Enterprise OrgIDをデータ要素に正しく入力してください。
...
CX Enterprise Visitor IDをディメンションとして使用し、「ボット」セグメントを適用することを忘れないでください。
