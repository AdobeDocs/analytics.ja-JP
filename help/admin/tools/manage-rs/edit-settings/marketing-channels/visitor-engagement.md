...
訪問者のエンゲージメント期間とは、訪問者の過去のサイト上のアクティビティを、どれだけの期間ファーストタッチチャネルに関連付けるかを示すものです。 デフォルトの有効期限設定は 30 日間です。
...
* 1 日目：ユーザーが Display でサイトにアクセスします。 ファーストタッチチャネルとラストタッチチャネルは「表示」に設定されます。
...
## マーケティングチャネルの有効期限を定義する {#define-expiration}
