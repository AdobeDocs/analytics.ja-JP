...
+ Analytics 管理ツール {#admin-tools}
  ...
  + レポートスイートマネージャー {#manage-report-suites}
    + レポートスイートの設定の編集 {#edit-report-suite}
      + 一般 {#report-suite-general}
        ...
        + ボットルール {#bot-removal}
          ...
          + [ボットルールを理解して設定する](tools/manage-rs/edit-settings/general/bot-removal/bot-rules.md)
      ...
      + コンバージョン {#conversion-variables}
        ...
        + ユニーク訪問者変数 {#unique-visitor-variable}
          ...
          + [ユースケース - 訪問者 ID の抽出](tools/manage-rs/edit-settings/conversion-var-admin/unique-visitor-variable-admin/extract-visitorids-usecase.md)
    ...
    + [レポートスイートの検索条件を保存](tools/manage-rs/t-report-suite-saved-search.md)
    + [レポートスイート設定をダウンロード](tools/manage-rs/t-download-rs-settings.md)
  ...
  + サーバーコールの使用状況 {#server-call-usage}
    ...
    + [現在のサーバーコールの使用状況を表示する](tools/server-call-usage/server-call-usage-dashboard.md)
    + [レポートスイートの使用状況を表示する](tools/server-call-usage/report-suite-usage.md)
  ...
  + ユーザーと製品の管理（レガシー） {#user-product-management}
    ...
    + Adobe Admin Console へのユーザーの移行 {#migrate-users}
      ...
      + [Enterprise ID および Federated ID 用に Analytics ユーザーアカウントを移行する](tools/user-management/user-migration/migrate-enterprise.md)
