...
* **302件見つかり**: この応答を受け取る理由はいくつか考えられます。
   ...
   * ComscoreとAdobeの統合： Comscore/Analytics統合を使用している場合、各イメージリクエストの結果は常に302となります。
* **404が見つかりません**: この応答は、イメージリクエストが見つからず、データがAdobeデータ収集サーバーに送信されないことを意味します。 この応答は、ハードコードされたイメージ要求が正しく形式設定されていない場合にも発生します。 この問題を解決するには、Analyticsを導入した個人またはチームと協力します。
