...
>[!WARNING]
>
>[`t()`](t-method.md)または[`tl()`](tl-method.md)のようなトラッキング呼び出しを`registerPostTrackCallback`変数内で行わないでください。 この変数でトラッキング呼び出しを設定すると、イメージリクエストの無限ループが発生します。
...
## Web SDK拡張機能を使用したコールバックの追跡

近日リリース予定

## Web SDKを手動で実装する事後コールバック

JavaScript Promiseを使用すると、データがAdobeに正常に送信された後、イベントを送信して関数を登録できます。
