...
The `charSet` 変数は、Adobeが受信データを UTF-8 に変換して、Analytics での保存とレポートを可能にするために使用します。 ほとんどのサイトでは、この変数を設定する必要はありません。
...
文字セットは、 [!UICONTROL 一般] アコーディオンを使用して、Adobe Analytics拡張機能をAdobe Experience Platformデータ収集で設定する際に使用します。
...
## AppMeasurementと Analytics 拡張機能のカスタムコードエディターの s.charSet
