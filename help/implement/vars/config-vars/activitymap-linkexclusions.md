...
description: リンク名でActivity Mapデータをフィルタリングします。
...
`ActivityMap.linkExclusions` 変数を使用すると、[Activity Mapリンク ](/help/components/dimensions/activity-map-link.md) ディメンション内のテキストに基づいて、Activity Mapデータを選択的にフィルタリングまたは除外できます。
...
`s.ActivityMap.linkExclusions` 変数は、Activity Mapトラッキングから除外するフレーズのコンマ区切り値を含む文字列です。 いずれかのフレーズが [Activity Mapリンク ](/help/components/dimensions/activity-map-link.md) ディメンションで収集された値と一致する場合、すべてのActivity Mapデータがヒットから削除されます。 この変数は、`linkUrl` ではなく `linkName` を参照することに注意してください。
