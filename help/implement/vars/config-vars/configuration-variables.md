...
* **`visitorMigrationKey`**：訪問者をサードパーティ Cookieからファーストパーティ Cookieに移行するために使用されるキーが含まれています。 最新のライブラリが1st パーティフォールバッククッキー（`fid`）を設定し、IDにExperience Cloud ID サービスを利用しているため、この機能は廃止されました。
...
* **`visitorNameSpace`**: サードパーティ Cookie ドメインの決定に役立ちました。 Experience Cloud ID サービスを使用しない実装に[`trackingServerSecure`](trackingserversecure.md)変数を使用することを推奨します。
