...
動的変数は、変数間で値をコピーできる簡単な概念です。 動的変数は、変数の値が長い場合に、イメージリクエストの URL の長さを短くするのに役立ちます。 この概念について詳しくは、[動的変数](../page-vars/dynamic-variables.md)を参照してください。
...
このフィールドにはデフォルトで `D=` 値が含まれています。 別の動的変数の接頭辞を使用する場合は、その値を変更できます。 サイトの文字コードと一致する限り、任意の値を使用できます。
