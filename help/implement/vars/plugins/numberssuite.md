...
`zeroPad` 関数は次の引数を使用します。

* **num**（必須、整数）：埋め込む数値です。この関数は、この引数に小数が含まれる場合、その値を切り捨てます。
* **nod**（必須、整数）：最終戻り値の桁数です。pad の桁数がパッド対象の桁数より小さい場合、プラグインは `num` 引数の先頭に 0 を追加します。

`randomNumber` 関数は次の引数を使用します。

* **nod**（オプション、整数）：生成する乱数の桁数です。最大値は 17 桁です。デフォルト値は 10 桁です。

`twoDecimals` 関数は次の引数を使用します。

* **val**（必須、数値）：最も近い小数点以下 2 桁に丸める数値（文字列オブジェクトまたは数値オブジェクトで表されます）です。

## 戻り値

* **zeroPad** 関数は、`num` 引数と同じ、値の先頭に特定の数のゼロを追加した文字列を返します。これにより、戻り値の桁数が正しくなります。
* **randomNumber** 関数は、任意の桁数の乱数と等しい文字列を返します。
* **twoDecimals** 関数は、最も近い小数点以下 2 桁に丸めた数値オブジェクトを返します。
