...
`getNewRepeat` プラグインを使用すると、サイトへの訪問者が新しい訪問者か、再訪問者かを希望の日数以内に判断できます。カスタム日数を使用して訪問者を「新規」として識別する場合は、このプラグインを使用することをお勧めします。Analysis Workspaceの「[!UICONTROL  新規訪問者 ]」セグメントと「[!UICONTROL  リピート訪問者 ]」セグメントが組織のニーズを満たす場合、このプラグインは不要です。
