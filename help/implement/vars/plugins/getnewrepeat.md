...
`getNewRepeat` プラグインを使用すると、サイトへの訪問者が新しい訪問者か、再訪問者かを希望の日数以内に判断できます。 カスタム日数を使用して訪問者を「新規」として識別する場合は、このプラグインを使用することをお勧めします。 Analysis Workspaceの[!UICONTROL 初回訪問回数]および[!UICONTROL 再訪問回数] セグメントが組織のニーズを満たしている場合、このプラグインは不要です。
...
1. 次の設定で、必要なデータ要素名を設定します。
   ...
   * データ要素：`getNewRepeat`
1. 右側の`daysBeforeReset` パラメーターを設定します。
