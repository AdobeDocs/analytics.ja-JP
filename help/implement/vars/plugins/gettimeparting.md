...
`getTimeParting` プラグインを使用すると、サイトで測定可能なアクティビティが発生した時間の詳細を取り込むことができます。 このプラグインは、指定した日付範囲内で、繰り返し可能な時間の区分ごとに指標を分類したい場合に役立ちます。 例えば、すべての日曜日とすべての木曜日など、2 つの異なる曜日間のコンバージョン率を比較できます。 また、すべての朝とすべての晩など、1 日の時間帯を比較することもできます。
...
1. 左側の「**[!UICONTROL タグ]**」をクリックし、目的のタグプロパティをクリックします。
1. 左側の&#x200B;**[!UICONTROL 拡張機能]**&#x200B;をクリックし、**[!UICONTROL カタログ]** タブをクリックします
1. **[!UICONTROL Common Web SDK Plugins]**&#x200B;拡張機能を探してインストールします。
1. 左側の&#x200B;**[!UICONTROL データ要素]**&#x200B;をクリックし、目的のデータ要素をクリックします。
1. 次の設定で、必要なデータ要素名を設定します。
   * 拡張機能：共通のWeb SDK プラグイン
   * データ要素：`getTimeParting`
1. 右側の`Time Zone` パラメーターを設定します。
1. データ要素に変更を保存して公開します。

## Web SDKを手動で実装するプラグインのインストール

このプラグインは、Web SDKの手動実装内での使用に対してはまだサポートされていません。
...
* Internet Explorer ブラウザーでの問題を修正しました。 時間を返すことができますが、訪問者のローカルタイムでのみ返すことができます。
...
* まったく新しいソリューションを提供し、年、月、日付の情報を含むようになりました。
