...
>[!CAUTION]
>
>このプラグインの以前のバージョンは、今後の年数に対応するわけではありません。 このプラグインの以前のバージョンを使用する場合は、JavaScriptのエラーやデータ損失を防ぐため、Adobeでは最新バージョンにアップグレードすることを強くお勧めします。 このプラグインをアップグレードできない場合は、プラグインコードの`s._tpdst`変数に将来の適切な年が含まれていることを確認してください。
