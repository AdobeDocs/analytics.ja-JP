...
このプラグインは、ブラウザーセッションの終了時に有効期限が切れる3つのファーストパーティ Cookieを作成します。

* `s_ppv`：関数を呼び出して公開された各値を保存します
* `s_tp`：前のページの合計ピクセル高さを保存します
* `s_ips`：前のページの最初のスクロール率を保存します
...
* 特定の特殊文字によってプラグインが破損する問題を修正しました
