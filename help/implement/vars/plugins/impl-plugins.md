...
1. Adobe Experience Platformのタグを使用した「Common Analytics Plugins」拡張機能の使用
2.  カスタムコードエディターを使用したプラグインコードの貼り付け。
...
1. まず、Analytics トラッキングオブジェクトをインスタンス化します（[`s_gi`](../functions/s-gi.md) を使用）。
   * タグが有効なサイトは、Adobe Analyticsが読み込まれると、トラッキングオブジェクトを自動的にインスタンス化します。
...
2. 次に、プラグインコードを含めます。
   ...
   * 実装でAdobe Experience Platformでタグを使用しない場合は、 `AppMeasurement.js`でトラッキングオブジェクトをインスタンス化した後の任意の場所にプラグインコードを貼り付けることができます。
3. 3 番目にプラグインを呼び出します。
   * タグを有効にしたサイトの内部と外部の両方にわたるすべての実装で、JavaScriptを使用してプラグインを呼び出します。 プラグインのページに記載されている形式を使用してプラグインを呼び出します。
4. 実装を検証し、発行します。
