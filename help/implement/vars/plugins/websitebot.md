...
# Adobeプラグイン：websiteBot

>[!IMPORTANT]
>
> このプラグインはアドビコンサルティングによって提供されており、Adobe Analytics からより多くの価値を引き出すのに役立ちます。アドビカスタマーケアは、インストールやトラブルシューティングを含め、このプラグインに対するサポートをおこないません。このプラグインに関するヘルプが必要な場合は、貴社のアカウントマネージャーにお問い合わせになって、担当コンサルタントとのミーティングを手配してもらってください。

`websiteBot`プラグインを使用すると、デスクトップ訪問者がボットかどうかを動的に識別できます。 このデータを使用すると、あらゆる種類のレポートの精度を高めることができ、合法なサイトトラフィックをより良く測定できます。

このプラグインは、次の2つのチェックを実行します。

* まず、`navigator.UserAgent`変数を使用して、デバイスがデスクトップかモバイルデバイスかを判断します。 モバイルデバイスは無視されます。
* デスクトップデバイスの場合は、マウスの動きに関するイベントリスナーを追加します。
...
Adobeでは、このプラグインを使用する前に次の点を推奨します。

* **eVar設定の指定**:レポートスイート設定の「 [コンバージョン](/help/admin/admin/conversion-var-admin/conversion-var-admin.md) 変数」でeVarを設定します。有効期限を&#x200B;**&quot;訪問者&quot;**&#x200B;に設定し、**&quot;元の値（最初）&quot;**&#x200B;に割り当てます。
* **別の変数でのユーザーエージェントの収集**:別の変数にユーザーエージェント文字列を収集して、このプラグインの有効性を監視します。このeVarを収集するには、すべてのヒットで`navigator.UserAgent`にデータを設定します。
