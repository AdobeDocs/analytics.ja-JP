...
description: 2 つのイベント間の時間を測定します。
...
`getTimeBetweenEvents` プラグインを使用すると、買い物かごやカスタムイベントを含む、2 つの Analytics イベントの間の時間を追跡できます。 これは、チェックアウトプロセスの完了に要する時間や、時間を測定したいその他のプロセスをトラッキングする場合に役立ちます。 このプラグインは、時間を測定する必要があるコンバージョンプロセスがない場合には、必要ありません。
...
1. 次の設定を使用して、上記のルールにアクションを追加します。
   ...
   * アクションタイプ：getTimeBetweenEvents を初期化
...
* **`ste`**（必須、文字列）：タイマー開始イベント。 タイマーを開始する Analytics イベントを指定する、コンマ区切りの文字列。
...
* **`fmt`**（オプション、文字列）：秒数を返す時間の形式（デフォルトは何も返さない）
  ...
  * 設定されていない場合、戻り値の形式は次のルールに基づきます。
    * 1 分未満の値は、最も近い 5 秒刻みに丸められます。 例：10 秒、15 秒
    * 1 分～1 時間の値は、最も近い 0.5 分刻みに丸められます。 例：30.5 分、31 分
    * 1 時間～1 日の値は、最も近い 0.25 時間刻みに丸められます。 例：2.25 時間、3.5 時間
    * 1 日を超える値は、最も近い 1 日刻みに丸められます。 例：1 日、3 日、9 日
