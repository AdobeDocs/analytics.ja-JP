...
## アドビでサービスの中断が発生した場合、パフォーマンスに影響する可能性はありますか。

いいえ。 JavaScript ファイルはアドビのサーバーでホストされていないため、アドビの障害が AppMeasurement ライブラリに影響することはありません。 Adobe Experience Platform のタグを使用する場合、JavaScript ファイルは Akamai でホストされるか、または組織が決定したサーバー上の場所でホストされます。

## ブラウザーからアドビのサービスにデータを送信することによって、パフォーマンスが低下することはありますか。

AppMeasurement は HTML ページ内にイメージオブジェクトを作成し、ブラウザーはアドビ データ収集サーバーからそのイメージオブジェクトをリクエストします。 データ収集サーバーが低速になったり応答しなくなった場合、そのリクエストを処理するスレッドは、イメージが返されるかタイムアウトが発生するまで、遅延します。 ブラウザーはイメージを複数のスレッドで処理するので、アドビの停止状態はページ読み込み時間にわずかな影響しか与えず、1 つのスレッドが停止していても、他のスレッドは機能し続けます。

## Analytics 実装を無効にする、または削除する方法を教えてください。

組織によっては、契約の有効期限切れに伴って実装を削除したい場合や、サーバーコールの数を減らす目的で実装を削除したい場合があります。
...
>以下は禁止されています。
>
>* レポートスイートを無効な値に変更しないでください。アドビのサーバーに不要な負荷が発生します。
