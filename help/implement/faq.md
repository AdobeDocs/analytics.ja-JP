...
## コードアナライザを通じてAppMeasurementを実行し、`Math.random()`の使用に潜在的なセキュリティ上のリスクのフラグを付けました。 `Math.random()`は機密データと共に使用されていますか？

いいえ。`Math.random()`を使用する数値は、機密データのマスク、送信、受信には使用されません。 Adobeのデータ収集サーバーに送信されるデータは、基になるHTTPS接続のセキュリティに依存します。<!-- AN-173590 -->

AppMeasurementは、次の3つの主要な領域で`Math.random()`を使用します。
...
* **フォールバック訪問者ID**:訪問者IDをcookieから取得できない場合は、ランダムな訪問者IDが生成されます。AppMeasurementのこの部分では、`Math.random()`への2回の呼び出しを使用します。
