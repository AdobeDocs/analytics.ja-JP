...
* 配布には、Visitor ID Service Library 5.5.0とData Integration Library 9.6が含まれます。
...
* 訪問者ID サービスがオプトインで承認されていない場合に、オプトアウトを尊重するようにAppMeasurementを改善しました。 （AN-175259）
...
>[!NOTE]
>
>Visitor ID サービスを使用するお客様には、Visitor API 3.0以降が必要です。 関連付けられたコードライブラリ（`at.js`、`AppMeasurement.js`、など）が更新された場合は常に訪問者 API を最新バージョンにアップグレードすることをお勧めします。
...
>[!NOTE]
>
>JavaScript バージョン 1.6.3の次のリリースノートも参照してください。これには、Visitor ID サービスの更新された要件が含まれます。
...
>[!IMPORTANT]
>
>訪問者ID サービス *のバージョン 1.6.0では、JavaScript バージョン 1.6.3以降の* AppMeasurementが必要です。 Visitor ID サービスのバージョン 1.6.0にアップグレードする場合は、必ずAppMeasurement 1.6.3以降を使用してください。
...
* [訪問者 ID サービス](https://experienceleague.adobe.com/ja/docs/id-service/using/home)をサポートするため、`VisitorAPI.js` を同梱しました。
