...
ベストプラクティスとして、使用する引用符の種類と一致していることを確認してください。 一重引用符で文字列の開始を指定する場合は、一重引用符を使用して文字列を閉じる必要があります。
...
HTML には、空白を作成するための文字が複数あります。 これには、スペース、タブ、キャリッジリターン（または改行）が含まれます。 次の例をご覧ください。
...
* **最新バージョンのAppMeasurementを使用**: AppMeasurement ライブラリ 1.4.1以降では、画像リクエストが長すぎる場合にHTTP POSTを使用して画像リクエストを自動送信します。 このメソッドを使用して送信されるデータは、長さに関係なく切り捨てられません。
