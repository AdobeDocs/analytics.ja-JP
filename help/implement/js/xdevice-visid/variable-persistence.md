...
* `visitorID` 変数が設定され、ヒットに対して送信されると、Adobeは、一致する訪問者 ID を持つ他の訪問者プロファイルがないかを確認します。
...
まだ認識されていない新しい `visitorID` 値（上記の `u999`）を含む最初のデータ接続時に、新しいプロファイルが作成されます。前のプロファイルからの持続性値は、新しいプロファイルに転送されます。
