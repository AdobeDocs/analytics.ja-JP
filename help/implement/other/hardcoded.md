...
アドビが提供する AppMeasurement ライブラリは、ページ上に存在する変数をコンパイルし、イメージリクエストとしてアドビに送信します。 AppMeasurement ライブラリを完全に使用せずに、手動でアドビにイメージリクエストを送信することもできます。 この方法では、イメージリクエストとクエリ文字列を手動で作成する必要があります。
...
* `example.data.adobedc.net` は、[`trackingServerSecure`](/help/implement/vars/config-vars/trackingserversecure.md) 変数に含まれる値です。
...
* メールのレンダリングごとに、課金対象のサーバーコールが増加します。
* HTML をサポートし、画像を許可しているメールクライアントのみが追跡されます。 Microsoft Outlook など、一部のメールクライアントは、外部画像をデフォルトでブロックします。 これらの電子メールは、受信者が外部画像のダウンロードを選択するまで追跡されません。

イメージリクエストを含む Outlook メールを作成するには、以下を実行します。

1. HTML エディターを開きます。 HTML エディターが使用できない場合は、プレーンテキストエディターを使用することもできます。
...
イメージリクエストは 1x1 の透過ピクセルなので、電子メールは変更されないように見えます。 テスト用にイメージリクエストを表示する場合は、HTML ファイルを変更して、ボーダーや追加のテキスト、その他のコンテンツを追加します。

## FAQ

ハードコードされたイメージリクエストに関するよくある質問について説明します。
...
各クエリ文字列パラメーターの値は、URL エンコードされています。 URL エンコードは、通常 URL で使用できない文字を、使用できる文字に変換します。 例えば、スペース文字は `%20` に変換されます。 英数字以外の文字が URL エンコードされていることを確認します。 イメージリクエストがデータ収集サーバーに到達すると、アドビは自動的に値を URL デコードします。
