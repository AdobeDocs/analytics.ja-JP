...
<table id="table_1CFB53FE72E74CCB8CAA5D4E3873D286"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col2"> <p> <b>手動</b>：AppMeasurement コードを手作業で管理します。<span class="keyword">Analytics</span> の <span class="keyword">AppMeasurement</span> コードは、<span class="ignoretag"><span class="uicontrol">管理ツール</span>／<span class="uicontrol">コードマネージャー</span></span>からダウンロードできます。 </p> <p>「<a href="https://marketing.adobe.com/resources/help/en_US/sc/implement/appmeasure_mjs.html">JavaScript（新規）</a>」をクリックして、コードをローカル環境にコピーし、「<a href="/help/implement/other/dtm/c-aa-tool/library-management.md">ライブラリ管理</a>」の「<span class="wintitle">コードを編集</span>」フィールドに貼り付けます。 </p> <p>手動設定に固有のフィールドに入力します。 </p> 
    <ul id="ul_CFB6CE78AEB743EF8B47BAAC42E2DB0A"> 
     <li id="li_5B7046CD95AB416F8C113B381A264D91"> <p><b>実稼働アカウントID:(必 </b>須)データ収集用の実稼働アカウント。 Analytics の場合はレポートスイート ID になります。Dynamic Tag Management は、実稼動およびステージング環境に適したアカウントを自動的にインストールします。 </p> </li> 
     <li id="li_14E840FD79A0451BABEDD15DC0584768"> <p><b>ステージングアカウントID:(必 </b>須)開発またはテスト環境。 Analytics の場合はレポートスイート ID になります。ステージングアカウントには、実稼動用データとは別個のテストデータが保持されます。 </p> </li> 
...
1. 必要に応じてフィールドを編集します。
   ...
   次の表に、上記のように、Analyticsツールの作成時に使用できる要素と異なる要素のみを示します。 ただし、両方の表で説明されているように、ページ上の任意の要素を変更できます。
...
<table id="table_2B60CD109CFF4839AB7F91D61125EDFF"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>注：この設定を有効にすると、手動で設定した実装が、「<span class="term">設定メソッド</span>」で説明した自動設定メソッドに代わります。 </p> <p>このオプションを選択すると、Dynamic Tag Management によって <span class="keyword">Adobe Analytics</span> アカウントの設定が自動取得されます。 </p> <p>利用可能な最新のAppMeasurementコードが使用され、新しいバージョンが利用可能になると、アップグレード通知が表示されて選択できます。 また、必要に応じて、以前のバージョンのAppMeasurementにロールバックすることもできます（互換性など）。 最大で 5 つの古いバージョンが表示されます。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>資格情報の更新 </p> </td> 
   <td colname="col2"> <p>例えば、APIを更新して、ユーザーに関連付けられたレポートスイートを更新します。 </p> </td> 
  </tr> 
 </tbody> 
</table>
