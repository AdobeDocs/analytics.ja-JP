...
Edge にはブックマークレットを手動で作成する機能はありませんが、ブックマーク URL は編集できます。
...
Safari ではブックマークレットを手動で作成することはできませんが、ブックマーク URL は編集できます。
...
* **わかりやすい形式**：イメージリクエストで役立つラベルと生のクエリ文字列の表示形式を切り替えます。詳しくは、[データ収集クエリのパラメーター](../js-implementation/data-collection/query-parameters.md)ーを参照してください。
