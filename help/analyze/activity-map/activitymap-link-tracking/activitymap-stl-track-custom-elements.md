...
s.tl()関数がHTML要素のオンクリックイベントから直接呼び出されると、Activity mapは、Webページの読み込み時にその要素のオーバーレイを表示できます。 例：
...
s.tl()関数がHTML要素のオンクリックイベントから直接呼び出されない場合、Activity mapは、ユーザーが要素をクリックした後にのみオーバーレイを表示できます。 s.tl() 関数が間接的に呼び出される場合の例を以下に示します。
...
Activity Map で動的コンテンツリンクをオーバーレイする最善の方法は、カスタマイズした ActivityMap.link 関数を設定して、戻り値が s.tl に渡される同じ関数を呼び出すことです。以下に例を示します。
