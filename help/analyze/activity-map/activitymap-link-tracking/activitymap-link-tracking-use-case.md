...
リンクまたはページ上のリンクの場所に対して一意のオブジェクトID `s_objectID`を作成することで、Activity Map追跡を向上させたり、リンクURLではなく、リンクタイプやリンクの場所に関するレポートをActivity Mapを使用したりできます。  変数について詳しくは、[ここ](https://docs.adobe.com/content/help/ja-JP/analytics/implementation/vars/page-vars/page-variables.html)をクリックしてください。`s_objectID`
...
各「購入」リンクに独自の領域が定義されていることを確認することで、領域をカスタマイズできます。 これを行うには、各「Buy」アンカータグの親の1つに`"id"`パラメーターを追加します。
