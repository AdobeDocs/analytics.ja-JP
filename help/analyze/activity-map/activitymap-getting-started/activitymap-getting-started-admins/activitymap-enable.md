...
> [!NOTE] 管理者以外のユーザーがダウンロードできるようにするに [!DNL Activity Map]は、「ツール」と「従来のClickMapのインストール」に対する権限を提供する新しいユーザーグループを作成します。 このレベルの権限は、Accessと組み合わせて、ツ [!DNL Activity Map] ールをダウンロードして使用する権限を提供します。
