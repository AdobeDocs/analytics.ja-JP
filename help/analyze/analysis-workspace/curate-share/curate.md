...
デモビデオについては、![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [&#x200B; プロジェクトをキュレーション &#x200B;](https://video.tv.adobe.com/v/328267?captions=jpn&quality=12&learn=on){target="_blank"} を参照してください。
...
1. （任意）他のコンポーネントを追加するには、共有するコンポーネントを左側のパネルからレポートスイートの **[!UICONTROL コンポーネントをキュレーション]** ドロップゾーンにドラッグします。
...
| **キュレートされたプロジェクト** | キュレートされていないすべてのプロジェクトコンポーネント | キュレートされていないすべてのプロジェクトコンポーネント | この役割が所有またはそれらと共有されている、キュレートされていないプロジェクトコンポーネント |
