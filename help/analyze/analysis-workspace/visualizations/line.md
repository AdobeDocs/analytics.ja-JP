...
>[!TIP]
>
>近似曲線は、今日（データの一部）や未来の日付を含まないデータに適用することをお勧めします。近似曲線は近似曲線をゆがめるためです。 ただし、未来の日付を含める必要がある場合は、データからゼロを削除して、それらの日の偏りを防ぎます。 これを行うには、ビジュアライゼーションのデータソーステーブルに移動し、指標列を選択して、 **[!UICONTROL 列の設定]** /ゼロを値なしとして **[!UICONTROL 解釈します]**。
