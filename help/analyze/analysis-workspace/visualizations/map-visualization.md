...
* 任意の指標（計算指標を含む）のビジュアルマップを作成することができます。
* 様々な地域にまたがる指標データを識別および比較する場合に役立ちます。
* モバイル使用状況からの緯度／経度または web 使用状況の地理的ディメンションという 2 つのデータソースをサポートできます。
* PDF の書き出しをサポートします。
...
1. ドロップダウンリストから、指標を選択します。 または、指標のリスト（計算指標を含む）から指標をドラッグします。
1. 取得するデータのデータソースを指定します。 このダイアログは、モバイルアプリデータの場所のトラッキングを有効にしている場合にのみ表示されます。
   ...
   | **[!UICONTROL 地理的ディメンション]** | このオプションは、訪問者の IP アドレスに基づいた訪問者の場所に関する地理的なセグメント化データを表します。 このデータは、 [!UICONTROL 国]、 [!UICONTROL 地域] および [!UICONTROL 市区町村] に変換されます。 DMA または郵便番号コードのレベルまでにはなりません。 ほとんどすべてのレポートスイートで、このディメンションが有効になっています。 お客様が使用していない場合は、Adobe カスタマーケアに連絡して、地理的レポートを有効にしてもらいます。 |
...
| **[!UICONTROL カスタム最大値]** | マップの最大値のしきい値を変更できます。この値を調整すると、カスタムの最大値として設定した値セットに対して、バブル／ヒートマップの値（色とサイズ）のスケールが調整されます。 |
