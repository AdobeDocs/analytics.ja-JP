...
キャンバス上に既に存在するノードに基づいて、上位の即時ノードを自動的に表示できます。 上位のノードをジャーニーキャンバスに追加したり、フリーフォームテーブルで表示したりできます。
