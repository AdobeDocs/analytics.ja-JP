...
* 静的コンポーネントが左側の列で使用され、[列の合計が行の合計として計算される場合](/help/analyze/analysis-workspace/visualizations/freeform-table/column-row-settings/table-settings.md)
...
Analysis Workspaceでは、自由形式テーブルのデータを次の種類の列で並べ替えることができます。

* 任意の指標列

* ディメンション列（文字列ベースのディメンションを除く）

複数の列で同時にソートすることもできます。
