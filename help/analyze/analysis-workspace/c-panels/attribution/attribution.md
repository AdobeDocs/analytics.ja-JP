...
>[!NOTE] 次のアルゴリズムアトリビューションモデルは、現在 [Adobe Analytics Labsで使用可能で](https://docs.adobe.com/content/help/ja-JP/analytics/analyze/tech-previews/overview.html) 、最終的には一般リリースの一部となります。
...
>[!NOTE] セグメントで指標を含むヒットが抑制される場合、その指標インスタンスはどのディメンションにも関連付けられません。 ただし、類似のレポートフィルターでは、一部のディメンション値が非表示になるだけで、アトリビューションモデルに従って処理される指標に影響を与えません。 その結果、同等の定義を持つセグメントとフィルターが、セグメントの値を低く返す場合があります。
