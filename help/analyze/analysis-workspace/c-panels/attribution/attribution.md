...
* **** カスタムルックバックウィンドウ：アトリビューションウィンドウを、レポートの日付範囲を超えて、最大90日まで拡大できます。 カスタムルックバックウィンドウは、レポート期間の各コンバージョンで評価されます。 例えば、2月20日に発生するコンバージョンの場合、10日のルックバックウィンドウでは、アトリビューションモデルの2月10日から20日までのすべてのディメンションタッチポイントが評価されます。
...
* 7日間のフ **ァーストタッ** チとカスタムのルックバックウィンドウを使用して ****、アトリビューションは最新の2回の訪問のみを調べます。 このレポートウィンドウでは、ソーシャルメディアリンクが最初に使用されたので、$50の購入に対して100%のクレジットを受け取ります。
...
* 線形 **とカスタ** ムのルックバック **（7日間）を使用して、クレジットはソーシャル** 、電子メール、表示の間で分割されます。 これらの各チャネルは、この購入に対して$12.50のクレジットを受け取ります。 有料検索は、指定されたルックバックウィンドウ内で発生しないので除外されます。
