...
これを支援するため、また、 [!UICONTROL 分析ワークスペースでの会社のデータコンポーネントの使用状況に基づいて、]Quick Insights  は、会社が使用する最も人気のあるディメンション、指標、セグメントおよび日付範囲を示すアルゴリズムを利用します。 実際、次に示すように、ドロップダウンリストには、 [!UICONTROL 人気の高い] 「閲覧済み」とタグ付けされたディメンション、指標、セグメントが表示されます。
