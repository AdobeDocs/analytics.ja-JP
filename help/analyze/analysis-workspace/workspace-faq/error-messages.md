...
| [!UICONTROL このディメンションでは、現在、デフォルト以外のアトリビューションモデルをサポートしていません。] | デフォルト以外の属性は、お使いのディメンションではサポートされていません。 | テーブル内のディメンションを、[Attribution ](/help/analyze/analysis-workspace/attribution/overview.md) と互換性のあるディメンションに置き換えます。 |
