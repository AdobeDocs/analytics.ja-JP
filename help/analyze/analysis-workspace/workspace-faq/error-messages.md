...
| [!UICONTROL このディメンションでは、現在、デフォルト以外のアトリビューションモデルをサポートしていません。] | デフォルト以外のアトリビューションは、使用しているディメンションではサポートされていません。 | テーブル内のディメンションを、[Attribution IQ](/help/analyze/analysis-workspace/attribution/overview.md) と互換性のあるディメンションに置き換えます。 |
