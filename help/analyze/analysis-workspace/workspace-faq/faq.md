...
+++Analysis Workspaceを使用するための前提条件は何ですか？
[Adobe Analytics 拡張機能を使用して Adobe Analytics にデータを送信](/help/implement/launch/validate-publish-prod.md)：Analysis Workspace を使用するには、有効な実装が必要です。ツールを使用する前に、組織がAdobeにデータを送信していることを確認します。 レガシーの手動による実装など、他の実装も同様に機能できます。
+++

+++Analysis Workspaceの管理およびアクセス要件は何ですか？
[管理要件](/help/analyze/analysis-workspace/workspace-faq/frequently-asked-questions-analysis-workspace.md)を参照してください。
+++

+++Analysis Workspaceを使用すると、データ収集に影響が出ますか。
...
+++1 つのAnalysis Workspaceプロジェクトには、いくつのレポートスイートを表示できますか。
...
+++どのようにAnalysis Workspaceを実装しますか。
...
+++Analysis WorkspaceはAdobe Analyticsの事前設定済みレポートを変更しますか。
...
+++指標をドラッグすると、「無効なデータ」と表示される。 この問題を解決する方法を教えてください。
...
+++指標をドラッグすると、実際のデータは表示されず、ゼロのみが表示される。 この問題をトラブルシューティングするにはどうすればよいですか？
