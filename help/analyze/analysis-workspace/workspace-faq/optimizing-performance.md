...
| 同時クエリ | 組織が同時にAdobeを要求しているクエリの数。 | 組織が特定のレポートスイートに対して同時に実行するリクエストが多すぎます。クエリは、APIリクエスト、レポートUI(Analysis Workspace、Reports &amp; Analytics、Report Builderなど)、スケジュール済みプロジェクト、スケジュール済みレポート、スケジュール済みアラート、およびレポートリクエストを行う同時ユーザーから収集できます。 | レポートスイートのリクエストやスケジュールを、1 日を通じて均等に配分します。また、可能な場合は、リクエストをピーク外の時間に切り替えます。 月曜の朝、火曜の朝、毎月1月の始めはレポートのピーク時です。 |
| データセンターの容量 | Adobeデータセンター内で、自社および他の顧客が共有するレポート容量の量。 | 組織およびデータセンター内の他の組織が同時にクエリした数。 | Adobeは、スペースが許せば最適化します。 組織は設定した容量を受け取る権利を持ち、システムの負荷が軽い場合は、Adobeはお客様に対して、お客様が受け取る許容量を超える容量を移行します。 |
