...
| 同時クエリ | 組織が同時にAdobeを要求しているクエリの数。 各組織には最低5つの同時クエリの権利が付与されます。 | レポートに長い時間がかかる場合は、通常、他のレポートと共にキューに入っていることが原因です。 これは、組織が特定のレポートスイートに対して多くの同時リクエストを実行しようとしていることを意味します。 クエリは、APIリクエスト、レポートUI(Analysis Workspace、Reports &amp; Analytics、Report Builderなど)、スケジュール済みプロジェクト、スケジュール済みレポート、スケジュール済みアラート、およびレポートリクエストを行う同時ユーザーから収集できます。 | レポートスイートのリクエストやスケジュールを、1 日を通じて均等に配分します。また、可能な場合は、リクエストをピーク外の時間に切り替えます。 月曜の朝、火曜の朝、毎月1月の始めはレポートのピーク時です。 |
