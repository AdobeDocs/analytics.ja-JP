...
Adobe Analyticsでのアトリビューションの概念には、次の2つのコンポーネントが必要です。
...
| ![アルゴリズム](assets/algorithmic.png) | [アルゴリズム](algorithmic.md) | 統計的手法を使用して、選択した指標の最適なクレジット配分を動的に決定します。 | ビジネスに適したアトリビューションモデルを選択する際に、推測やヒューリスティックを回避するのに役立ちます。 |
...
>[!NOTE] **[!UICONTROL カスタムのルックバックウィンドウ]** は、現在制限付きのテスト中です。 詳しくは、 [Adobe Analytics機能リリース](/help/landing/an-releases.md) （英語）を参照してください。
