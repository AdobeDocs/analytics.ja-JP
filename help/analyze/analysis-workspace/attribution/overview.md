...
一部のレポートでは、有料検索に関連付けられた注文が必要な場合があります。 他のレポートでは、Socialに関連付けられた注文が必要な場合があります。 アトリビューションを使用すると、このレポートの側面を制御できます。 Adobe Analytics Ultimate、Prime、Select、Foundationのすべての組織で利用できます。 アドビとの契約のタイプが不明な場合は、組織のアカウントマネージャーにお問い合わせください。
...
* [分類へのアトリビューションの適用](../components/dimensions/t-breakdown-fa.md): 分類にデフォルト以外のアトリビューションを使用する。
