...
可能な場合、アトリビューションは、永続化された値ではなく、データソース内のイベント内で送信される MID 列の値に依存します。 アトリビューションモデルは、その場で、データソースの MID 列の値に適用されます。 例えば、「ラストタッチ」アトリビューションを使用する場合、モデルは指標の各インスタンスから開始し、MID 列に表示される最後の値にモデルが達するまで、ヒット内を順番に遡ります。

不可能な場合、アトリビューションは、評価のためにデータソースの「前のレコード」の MID 値を使用します。 AA が順序不正データをサポートしていない場合、この以前のレコードはタイムスタンプ順に並べ替えられない可能性があります。
