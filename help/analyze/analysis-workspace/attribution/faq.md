...
>[!NOTE] セグメントで指標を含むヒットが抑制される場合、これらの指標インスタンスはどのディメンションにも関連付けられません。 ただし、類似のレポートフィルターでは、単に一部のディメンション値を非表示にするだけで、アトリビューションモデルに従って処理される指標に影響を与えません。 その結果、セグメントは、同等の定義を持つフィルターよりも低い値を返すことができます。
