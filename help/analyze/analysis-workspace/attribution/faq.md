...
+++##アトリビューションモデルは、データフィードやData Warehouseなど、他の Analytics 機能で使用できますか。
...
+++##アトリビューションモデルを使用できるのは、レポート時間処理を有効にして仮想レポートスイートを使用している場合だけですか。
...
はい、ほとんどのデータソースがサポートされています。これらのデータソースは Analytics の訪問者の識別子と結び付けられないので、サマリレベルのデータソースではアトリビューションを使用できません。

トランザクション ID のデータソースは他のヒットと同様に扱われます。 トランザクション ID データソースは、従来のレポートで通常使用される特殊な処理を使用しません。 つまり、レポートの時間処理を使用する場合、トランザクション ID のヒットでは、トランザクション ID のヒットのタイムスタンプの近くで発生するヒットから伝播されるeVar値が含まれます。 値は、元のトランザクションの時間の近くに発生したヒットからは伝播されません。

可能な場合、Attribution IQ は、永続化された値ではなく、データソース内のイベント内で送信される MID 列の値に依存します。 アトリビューションモデルは、その場で、データソースの MID 列の値に適用されます。 例えば、「ラストタッチ」アトリビューションを使用する場合、モデルは指標の各インスタンスから開始し、MID 列に表示される最後の値にモデルが達するまで、ヒット内を順番に遡ります。

不可能な場合、Attribution IQ は、評価のためにデータソースの「前のレコード」の MID 値を使用します。 AA が順序不正データをサポートしていない場合、この以前のレコードはタイムスタンプ順に並べ替えられない可能性があります。

レコードが順番に並べられていないので、永続性を適用した場合の期待値は、指定されたトランザクション ID のタイムスタンプと元のトランザクションの間に存在する時間に影響を与える可能性があります。

+++


+++##アトリビューションはAdvertising Analyticsとの統合に対応していますか。
...
マーケティングチャネルが初めて導入された際には、ファーストタッチとラストタッチのディメンションのみが導入されました。現在のバージョンのアトリビューションでは、明示的なファーストタッチディメンションとラストタッチディメンションは不要になりました。Adobeは汎用 [!UICONTROL マーケティングチャネル] および [!UICONTROL マーケティングチャネルの詳細] ディメンションを使用して、目的のアトリビューションモデルで使用できます。 これらの汎用ディメンションは、と同じように動作します。 [!UICONTROL ラストタッチチャネル] ディメンションのラベルは異なりますが、異なるアトリビューションモデルでマーケティングチャネルを使用する際の混乱を防ぐために、異なるラベルが付けられます。
...
例えば、「表示ヒット数」セグメントを適用した仮想レポートスイートを作成すると、一部のアトリビューションモデルを使用して、テーブル内の他のチャネルを表示できます。
