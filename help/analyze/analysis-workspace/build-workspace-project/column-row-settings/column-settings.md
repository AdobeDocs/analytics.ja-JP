...
[!UICONTROL 列の設定] ：列の書式設定を設定できます。列の書式設定の一部は条件付きにすることができます。

## 列設定 [!UICONTROL の編集] {#column-settings}
...
| 割合制限を使用 | 制限範囲を、絶対値ではなくパーセント値に基づくように変更します。 このオプションは、割合のみに基づいている指標（バウンス率など）のほか、カウントと割合を持つ指標（ページビュー数など）で使用できます。 |
| 自動生成 | データに基づいて上限/中間/下限を自動的に計算します。 上限は、この列の最大値です。下限は最小値であり、中間点は上限と下限の平均値です。 |
| カスタム | 上限/中間/下限を手動で割り当てます。 これにより、どのような場合に列の値が良好、平均的、望ましくないと判断されるかを柔軟に決定できます。 |
