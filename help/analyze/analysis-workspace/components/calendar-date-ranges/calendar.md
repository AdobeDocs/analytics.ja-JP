...
Workspace で作業している場合、日付範囲コンポーネントをパネルカレンダーに対して相対的に設定し、左側のパネル（またはコンポーネント内）にプレビューされたデータがパネルの日付範囲に基づいて表示されるようにできます。 相対パネルの日付が有効になる 3 つの一般的な使用例は、コンボグラフ、主要指標の概要、フリーフォームテーブルの日付範囲です。
...
1. 選択または選択解除 **パネルカレンダーに対する日付範囲コンポーネントの相対的な指定**.
   * 日付範囲コンポーネントをパネルカレンダーに対して相対的にするには、このオプションを選択します。
   * このオプションの選択を解除すると、パネルの日付範囲（主要指標の概要、コンボグラフ、紫の日付の丸薬）が変更されても、パネル内の日付範囲は更新されません。 これはデフォルトの設定です。

   ![パネルの相対日付](assets/relative-date-snippet.png)
