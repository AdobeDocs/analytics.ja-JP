...
1. 次のいずれかの方法を選択して、クイックセグメントの作成を開始します。
   ...
   * **アドホック（ドラッグ&amp;ドロップ）:** 左側のレールから、コンポーネントを **セグメント** パネルヘッダー内のアイコンをクリックし、 **編集** アイコンを使用してセグメントを調整します。
     ...
     > * 次のコンポーネントタイプはサポートされていません：計算指標およびディメンション、およびセグメントを作成できない指標。
...
> 
> * クイックセグメントを保存するには、 [Adobe Admin Console](/help/admin/admin-console/permissions/analytics-tools.md).
> 
