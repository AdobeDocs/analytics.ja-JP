...
例えば、適切なディメンション（この例では **[!DNL Marketing Channel]**）をタイルにドラッグ＆ドロップして、**[!UICONTROL ユニーク訪問者数]**&#x200B;指標に追加できます。Dimensionの分類は、 [!UICONTROL ドリルイン] 詳細スライド固有のセクション **[!UICONTROL プロパティ]**. タイルごとに複数のディメンションを追加できます。
...
スコアカードビルダーでタイルをクリックすると、右側のパネルにそのタイルとその詳細スライドに関連付けられているプロパティと特性が表示されます。 このレールで、新しい **タイトル** タイルの場合は、セグメントを適用してタイルを設定することもできます。

![プロパティタイル](assets/properties_tile.png)

## 詳細スライドを表示 {#view-detail-slides}

タイルをクリックすると、動的なポップアップウィンドウに、アプリ内のエグゼクティブユーザーに詳細スライドがどのように表示されるかが表示されます。 ディメンションを追加して、特定のニーズに合わせてデータを分類できます。 ディメンションが適用されていない場合、分類ディメンションは次のようになります **時間** または **日**（デフォルトの日付範囲に応じて）

分類を使用すると、指標をディメンション項目で逐語的に分類することで、分析を絞り込むことができます。
