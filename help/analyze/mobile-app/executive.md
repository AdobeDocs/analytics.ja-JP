...
次の情報は、Adobe Analyticsダッシュボードの使用と表示に関するベストプラクティスに関する情報をエグゼクティブユーザーに提供します。 この情報をデモするビデオを見るには、 [Adobe Analyticsダッシュボードのアプリ内エクスペリエンスを参照してください](https://youtu.be/QXqQ_PkArbA)。
...
ダウンロードとインストールが完了すると、エグゼクティブユーザーは、既存のAdobe Analyticsの資格情報を使用してアプリにログインできます。
...
5. タイルをタップして、そのタイルの詳細な分類の仕組みを確認します。
...
7. スコアカードにセグメントフィルタを適用するには、フィルタドロップダウンをタップし、キュレータが設定したセグメントを選択します。 [アプリのフィルター](https://docs.adobe.com/content/help/en/analytics-learn/tutorials/analysis-workspace/using-panels/using-drop-down-filters.html) は、Analytics Workspaceと同じように機能します。

   ![セグメントフィルター](assets/segment_filter.png)

8. スコアカードの更新：スコアカードに興味のあるすべての指標や内訳が含まれていない場合は、Analytics チームに連絡してスコアカードの更新を依頼してください。更新されたら、画面上のカードをプルダウンすると、最近追加したデータをロードして更新を表示できます。
...
10. 環境設定を変更するには、上に示す「 **環境設定** 」オプションをタップします。 環境設定で、生体認証ログインをオンにするか、次に示すようにアプリケーションのダークモードを設定できます。
