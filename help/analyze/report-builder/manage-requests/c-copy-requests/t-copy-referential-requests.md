---
description: 参照リクエストのコピー方法を説明します。
title: 参照リクエストのコピー方法
...
参照リクエストをスプレッドシートに反映またはコピー&amp;ペーストするには、次の手順に従います。
* スプレッドシート内に有効なリクエストを少なくとも 1 つ作成する必要があります。
* リクエストで生成されるデータには、値が別のセル内のリクエスト（分類または一致フィルターを使用）に依存するセル、またはセルに入力されたデータから入力を受け取るフィルターに依存するセルが含まれている必要があります。
...
>[!NOTE]
>
>1 つのリクエストを複数のワークシートに出力することはできません。 また、コピーしたリクエストの一部は、他のワークシートの入力フィルターが含まれているので、新しいワークブックに貼り付けることができません。 入力フィルターには、セルのレポートスイート、セルの日付範囲、セルのフィルター、その他の関連パラメーターが含まれます。
