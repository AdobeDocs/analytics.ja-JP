---
description: 参照リクエストをコピーする方法を説明します。
title: 参照リクエストをコピーする方法
...
参照リクエストをスプレッドシートに反映またはコピーして貼り付けるには：
* スプレッドシートに少なくとも 1 つの有効なリクエストを作成する必要があります。
* リクエストによって生成されるデータには、値が、別のセルのリクエスト（分類フィルターまたは一致フィルターを使用）か、セルに入力されたデータから入力を取得するフィルターに依存するセルが含まれている必要があります。
...
>[!NOTE]
>
>1 つのリクエストを複数のワークシートに出力することはできません。 また、リクエストに他のワークシートからの入力フィルターが含まれているので、コピーされたリクエストの一部を新しいワークブックに貼り付けることはできません。 入力フィルターには、セルのレポートスイート、セルの日付範囲、セルのフィルター、その他の関連パラメーターが含まれます。
