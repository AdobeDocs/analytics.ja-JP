---
description: カスタムレイアウトで、分類やマイクロチャートの追加など、ピボットレイアウトの機能と柔軟性をどのように提供するかを説明します。
title: Adobe Analyticsでのカスタムレイアウトの設定方法
uuid: 4c1404b1-f20e-4343-be97-0b4c03500d52
feature: Report Builder
role: User, Admin
exl-id: b3f71fd2-4081-4eaa-a9d6-4007446c1802
TQID: https://experienceleague.adobe.com/KnIi9Ft8W-7GV1Wkgs3Yj6QW-Y7eTktRmWhhPAJEYGI
product_v2: id: e55547f1-a1ff-40c6-8978-026e40ab7fa4
feature_v2: id: b3f03848-ae12-48b2-8aab-cad18567eb32id: c153fd90-23e1-4614-81d3-3cc7571227f7id: f73667dc-d296-4875-8975-ac3fdc3adc42
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
source-git-commit: ff16e07c7a2b75e9c6cc09e8255a7ea7e4c6f0c8
workflow-type: tm+mt
source-wordcount: 91
ht-degree: 14%

---

# カスタムレイアウトの設定

{{legacy-arb}}

カスタムレイアウトでは、ピボットレイアウトの機能の大部分を使用できますが、グリッド内の各項目をスプレッドシート内のどこに配置するかを選択できます。 このレイアウトは、分類やマイクロチャートの追加など、以前のリリースで利用可能な柔軟性を提供します。

次のトピックは、[!UICONTROL  カスタムレイアウト ]に固有です。

* [指標およびディメンションのセルへのマッピング](/help/analyze/legacy-report-builder/layout/map-metrics-and-dimensions-to-cells.md)
* [マイクロチャートの作成](/help/analyze/legacy-report-builder/layout/t-create-a-microchart.md)
