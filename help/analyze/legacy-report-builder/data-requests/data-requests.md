...
リクエストウィザード：ステップ 1 のフォームでは、レポートスイート、レポートタイプ、セグメントを選択し、日付を設定します。
...
特にモニターの設定を切り替えて使用しているユーザーの場合、リクエストウィザードが画面外に表示されることがあります。 例えば、職場ではドッキングステーションを、自宅ではノートパソコンの画面を使用するとします。 リクエストウィザードが既に開いている状態で「作成」を再度クリックすると、次のエラーが表示されます。

「新しいリクエストウィザードを開始する前に、まずリクエストウィザードの処理を完了する必要があります。」

リクエストウィザードを画面上に戻すことで、この問題は解決します。
...
4. `[M]` を押します。.
5. いずれかの矢印キーを押します。
6. マウスを移動します。これにより、リクエストウィザードがカーソルに追従します。
7. マウスをクリックして、リクエストウィザードを画面上に表示します。
