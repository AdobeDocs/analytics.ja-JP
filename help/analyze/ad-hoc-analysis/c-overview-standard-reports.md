...
これらのレポートは通常カスタマイズされるので、分析ごとに異なります。See the [Admin Console help](https://helpx.adobe.com/jp/enterprise/using/admin-console.html) for more information.
...
| Sum | データセットの各行の値の合計です。例えば、ある訪問者による訪問の回数の合計が算出されます（訪問者を訪問回数にかかわりなく 1 回だけカウントするのとは異なります）。この値は、収集したデータポイントの総合計になります。 |
