...
| **[!UICONTROL 新規作成]** モーダルが戻りました | クリック時 **[!UICONTROL 新規作成]** Workspace で、 [!UICONTROL 空のプロジェクト] および [!UICONTROL 空のモバイルスコアカード]. 会社が作成した任意のテンプレートから選択することもできます。 | ![新規作成](assets/create-new.png) |
...
| [!UICONTROL プロジェクトを作成] | ドロップダウンメニューを使用して、[!UICONTROL ワークスペースプロジェクト]または[!UICONTROL モバイルプロジェクト]を作成できます。 |
...
| 質問 | 回答 |
| --- | --- |
| に表示されていたテンプレートはどこにありますか。 [!UICONTROL Workspace]? | これらのテンプレートは、「[!UICONTROL レポート]」タブの下にグループ化されています。 |
| ベータ版をオプトアウトできますか？ | ベータ版はいつでもオプトアウトできます。 |
| ベータ版プログラムの UI で行う作業は、実稼動環境に引き継がれますか。 [!UICONTROL Workspace] 経験？ | はい、ベータ版で行われた作業は、古い/現在の [!UICONTROL Workspace] エクスペリエンス。 |
| 現在 [!DNL Reports & Analytics] お気に入りは持ち越されましたか？ | いいえ、引き継がれません。ただし、 [!UICONTROL Workspace] プロジェクトのお気に入りは引き継がれます。 |
