...
| 次を含む | 入力された値のサブ文字列と比較される項目を返します。例えば、「Page」のルールが「Search」を含む場合、「Search Results」、「Search」および「Searching」など、サブ文字列「Search」を含むページと一致します。Adobe Analyticsでは、「contains」句では大文字と小文字が区別されませんが、Customer Journey Analyticsでは大文字と小文字が区別されます。 |
...
| 一致する | 指定された数字または文字列の値に基づいて完全に一致する項目を返します。Adobe AnalyticsおよびCustomer Journey Analyticsでは、「一致する」句では大文字と小文字が区別されます。 **注意**:ワイルドカード（グロビング）機能を使用する場合は、この演算子を使用します。「グロビング」の例：<ul><li>`a*e` は、 `ae`、、、、 `abcde` `adobe`と一致します。  `a whole sentence`</li><li>`adob*` は、およびと `adobe`一致しま `adobe analytics`す。  `adobo recipe`</li><li>`*dobe` は、およびと `dobe`一致しま `adobe`す。  `cute little dobe`</li></ul> |
