...
デモビデオについて詳しくは、![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [セグメントコンテナ](https://experienceleague.adobe.com/ja/docs/analytics-learn/tutorials/components/segmentation/segment-containers){target="_blank"}を参照してください。
...
訪問者コンテナには、指定した期間内の訪問者に対するすべての訪問とページビューが含まれます。 訪問者レベルでのセグメントは、条件に加え、訪問者が表示した他のすべてのページ（および定義された日付範囲によって制限されたページ）を満たすページを返します。 訪問者コンテナレベルで生成されたレポートは最も広義のコンテナとして、すべての訪問のページビューを返し、複数回の訪問にわたる分析の生成を可能にします。したがって、訪問者コンテナは、定義された日付範囲に基づく変化の影響を最も受けやすくなります。
...
ヒットコンテナは、セグメントに含めるまたは除外するページヒットを定義します。ヒットコンテナは、条件が成り立つ特定のクリックやページビューの識別に使用できるコンテナの中で最も範囲が狭いものです。単一のトラッキングコードを表示したり、サイトの特定のセクション内での行動を分離したりできます。また、注文時のマーケティングチャネルなど、アクションが発生した場合に特定の値を特定することもできます。

ヒットコンテナには、単一ページの分類に基づいた値が含まれます。
...
論理グループコンテナを使用すると、セグメントルール内に別のコンテナを提供して、階層に基づかないエンティティをフィルタリングできます。 例えば、訪問者に基づいてフィルターを適用する、セグメント内にネストされたコンテナを提供できます。 この論理タイプでは、（トップレベルの訪問者コンテナを既に使用したので）階層を超えて選択した訪問者のみを選別する必要があります。詳しくは、[論理グループの例](/help/components/segmentation/segmentation-workflow/seg-sequential-build.md)を参照してください。

## コンテナのネスト {#nest-containers}

他のコンテナ内にセグメントコンテナを作成する場合は、本質的にセグメント内にセグメントを作成します。 ネストされたコンテナでは、次のロジックが使用されます。

1. どのようなデータが含まれているかを、最も外側にあるコンテナによって調べます。この外側のルールに一致しないデータは、セグメント化されたレポートでは破棄されます。
1. ネストされたルールを残りのデータに適用します。 ネストされたルールは、最初のルールがスローするヒットには適用されません。
1. ネストされたすべてのコンテナルールが計算されるまで繰り返します。 残りのデータは、結果のレポートに含まれます。

>[!NOTE]
>
>セグメント内にセグメントをネストすると（例えば、コンポーネント パネルからセグメント定義にセグメントをドラッグすると）、コンテナは、ドラッグしたセグメントルールのコピー（参照ではなく）とともに作成されます。

ネストは、コンテナ間でもコンテナ内のルール間でも使用できます。各コンテナには、以下をネストすることができます。
...
新しい複合セグメントに複数のセグメントを含めると、データをさらに絞り込むことができます。 既存の 2 つのセグメントをドラッグすると、訪問者をフィルタリングする際に「OR」文として機能します。 キャンバス内のすべてのコンテナはすべてのデータに対してレビューされ、いずれかのコンテナに一致するデータがレポートに含まれます。
...
コンテナを使用すると、セグメントを分類してレポートに適用する際に、レポート値に基づいて異なるデータをフィルタリングできます。

訪問者/訪問/ヒットコンテナ階層の各レベルで取得されるデータは、セグメントの作成方法に影響します。 同じデータセットを使用して、同じセグメントを同じレポートに適用しても、レポートの生成時に基になるコンテナに応じて異なる値が得られます。コンテナレポートのレベルや、ヒット全体での値の永続化などの要素が、レポートの精度に大きな変化をもたらす可能性があります。

### コンテナデータの基本事項 {#container-data}

例えば、以下に示す訪問者は、最初の訪問でサイトを訪問し、ホームページに移動した後、3 つの追加ページを訪問し、訪問をセールに変換しました。 別の訪問では、訪問者は製品ページ、ホームページ、製品ページに戻り、ウィンターハットを見た後にセッションを閉じました。 セグメントのコンテナごとに取り込まれたデータに応じて、レポートには様々な値が表示されます。
...
この条件がヒットコンテナ内にある場合、レポートには *Page = Winter Coats* が true のページのみがリストされます。 1 ページのみのコンテナでは、この条件に一致するページは 1 ページのみなので、Winter Coats ページのみが表示されます。
...
この同じ条件が訪問コンテナ内にある場合、レポートは、*Page equals Winter Coats* が true である訪問内のすべてのページをリストします。 Winter Coats ページをフィルタリングするだけでなく、条件が満たされる訪問の他のすべてのページもキャプチャします。 この訪問者は、ホーム、製品および購入のページも訪れているので、訪問者コンテナデータを使用してレポートを作成する場合、これらの追加ページもレポートに示されます。
...
多くのセグメントルールが 1 回の訪問あたりのページビューを識別します。このように識別された場合、単一のヒットがルールと一致するだけで、訪問者コンテナ全体が適用されます。訪問に基づくページビューは、1 回の訪問あたりのページビューに基づいたinsightを提供するので、このセグメントレポートは特に役立ちます。
...
分類スコープよりも小さいコンテナでセグメントを使用すると、予期しないデータが返されます。 より小さな分類を使用すると、そのデータスコープからすべてのヒットを引き込むことができます。

- **エントリページが製品ページと等しいヒットコンテナ**：すべてのページは訪問のエントリページに関連付けられ、訪問ベースの分類になります。 このセグメントを使用すると、入口となるページの製品ページだけでなく、その訪問のすべてのヒットも抽出されます。
- **リスト変数 1 に ValueA が含まれるヒットコンテナ**：リスト変数と同じヒットで複数の値が定義された場合、すべての変数値がセグメントに含まれます。 ヒットコンテナはヒットを分類する最小のセグメントコンテナなので、同じページビューに発生する値を区切る方法はありません。
- **ページが「購入」と等しいヒットコンテナ**：ページビューを指標として使用する場合、購入ページのみが（期待どおりに）表示されます。 売上高パーティシペーション レポートを使用する場合、パーティシペーション指標は訪問ベースなので、最初の訪問のすべてのページが 100 ドルを受け取ります。
- **ページが「ウィンターコート」に等しいヒットコンテナ**：ページビューを指標として使用する場合、ウィンターコートページのみが（期待どおりに）表示されます。 収益参加レポートを使用する場合、このディメンションには永続ディメンションが必要なので、ページにクレジットは割り当てられません。 実際に購入を行ったページビュー（購入ページ）はヒットコンテナに含まれないので、どの項目にも売上高の参加は与えられません。 ただし、訪問コンテナからレポートを実行すると、その訪問のすべてのページビューが含まれ、セッションで表示されたすべてのページに売上高のパーティシペーション（100 ドル）が配分されます。
...
セグメントデータは、ディメンションの永続性または選択したページに適用された変数に基づいて異なる場合があります。 ページディメンションなどの一部のディメンションは、ページレベルで一意の値を提供し、ヒットコンテナからのデータに基づいてフィルタリングされます。 （[コンテナデータに基づくレポート](/help/components/segmentation/seg-overview.md)の例を参照してください）。また、参照ドメインディメンションなどのディメンションは、訪問の複数のページ間で持続します。例：`Referring Domain equals aol.com`。訪問の期間など、一部のディメンションや適用された変数は、訪問者の履歴全体に及びます。

<!--![](assets/RefDomain_aol.png)-->

ページディメンションとは異なり、参照ドメインの値は、この訪問の各ページに添付されます。 例えば、以下の訪問者が参照元サイトからホームページに到達したとします。 したがって、この訪問のすべてのページに、同じ参照ドメイン値が割り当てられます。
...
ヒットコンテナからのデータを示すと、92,000 回を超えるページビューが 33,000 回を超える訪問で、32,000 人を超える訪問者によって表示されました。 平均して、各訪問で 3 つのページビューがあり、ほとんどすべての訪問はユニーク訪問者によるものでした。
