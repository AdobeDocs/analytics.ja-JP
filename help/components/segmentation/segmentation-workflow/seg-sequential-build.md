...
例えば、レストランでは、メインのランディングページを避け、注文ページに直接移動した常連ユーザーを確認したいとします。 この例では、訪問者がページ A にアクセスするのを避け、ページ B に進むのを簡単にすることができます。
...
例えば、衣料品店では、製品ページを閲覧したが、その後買い物かごを訪問しなかったすべての訪問者を表示したいと考えています。 この例を、現在の訪問または後続の訪問で、ページ A に移動した訪問者がページ B に移動しないように、簡略化できます。
