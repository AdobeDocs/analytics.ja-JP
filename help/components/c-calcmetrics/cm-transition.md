...
> [!NOTE] 計算指標が特定のレポートスイートに連結されなくなり、ログイン会社に表示される任意のレポートスイートで使用できる場合でも、計算指標が作成されたレポートスイートまたは最後に保存されたレポートスイートは計算指標マネージャに表示されます。

> [!NOTE] 計算指標を削除しても、その指標を参照するブックマークやダッシュボードレポートは引き続き機能します。
...
> [!NOTE] 計算指標がブックマーク、ダッシュボード、アラートまたは予定レポートで使用されていた場合、新しいコピーを編集しても、古い持続的な計算指標には影響しません。
...
現在は、ReportSuite CalculatedMetrics APIメソッド（1.3または1.4）は、古いストアを使用して引き続き計算指標の保存、削除および取得を行います。 既存の計算指標は移行され、新しい計算指標ビルダーに表示されます。**API メソッドを使用して作成される新しい計算指標はその API にのみ表示されます。このような指標は引き続きレポート API で使用できます。**
...
選択したレポートスイートに存在しないベース指標またはベースディメンションが計算指標に含まれている場合、ユーザーインターフェイスに「不明」と表示されます。
...
現在、マーケティングチャネルレポートの指標セレクターに表示されるのは、計算指標ビルダーで配分タイプが明確に「ファーストタッチ」または「ラストタッチ」に設定されている計算指標のみです。マーケティングチャネルレポートに適用済みの計算指標はこれまでどおり適用され、機能します。マーケティングチャネル用の計算指標を作成するには、計算指標ビルダーの設定アイコンをクリックし、配分タイプとして「ファーストタッチ」または「ラストタッチ」を選択します。これを行うと、計算指標はマーケティングチャネルレポートとのみ互換性があり、他のレポートでは使用できなくなるので注意が必要です。

## 一部の計算指標の数式に、追加した丸括弧が表示されないのはなぜですか？{#section_AC0D1E9714AD487F9A1C73359F518B5E}

移行時には、一部の数式から不必要な丸括弧が自動的に削除されます。削除されるのは、指標の計算に影響しない丸括弧のみです。これでデータは変更されません。式が単純化されるだけです。
...
リクエストがv5.2で作成され、そのリクエストに計算指標が含まれている場合、それらの指標はv5.1（以前のバージョン）では表示されません。 これは、計算指標がグローバル ID（レポートスイートに固有ではない ID）を使用するようになったためです。
...
[!UICONTROL Reports &amp; Analytics] (Reports &amp; Analytics)で計算指標の合計が表示される場合 は、単に数式を合計に適用するだけです。 例えば、注文件数/訪問件数という計算指標の合計は、合計注文件数を取得して合計訪問回数で割ります。ただし、計算指標の合計が行項目の単なる合計ではなく、サイトの合計になる場合があります。
