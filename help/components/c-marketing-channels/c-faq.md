...
* 除外処理を使用します。メールチャネルとアフィリエイトチャネルに同一のクエリ文字列パラメーターを使用しているとき、メールのトラッキングコードが少数なら、メールを定義するルールセットでメールトラッキングコードを指定することができます。その後、残りすべてのトラッキングコードを *`affiliates.`* として分類します。
