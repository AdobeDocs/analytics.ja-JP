...
* [滞在時間指標](/help/components/c-variables/c-metrics/metrics-time-spent.md#section_4F54D70300944748A62088F5870E4B6C)
* [滞在時間ディメンション](/help/components/c-variables/c-metrics/metrics-time-spent.md#section_D51606544CB046FC902E2E317318892C)
* [滞在時間の計算方法](/help/components/c-variables/c-metrics/metrics-time-spent.md#section_90A3882638974969A4B8B674FFDB7624)
* [滞在時間に関する FAQ](/help/components/c-variables/c-metrics/metrics-time-spent.md#section_51C2735BACAB42CCBA1DD3CBF238E2F7)
* [計算例](/help/components/c-variables/c-metrics/metrics-time-spent.md#section_3D63D6A601F34E42AD5366435CB610D5)
...
<table id="table_7095406DF1614F3CAD5E437B919598D1"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p><i>合計滞在時間（秒） / （実訪問者 - バウンス実訪問者数）</i> </p> <p>訪問者の全期間における、特定のディメンション項目に対する訪問者のインタラクション時間の平均を表します（cookieの長さ）。 </p> </td> 
   <td colname="col3"> <p>Analysis Workspace </p> <p>Reports &amp; Analytics </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>サイトでの平均滞在時間（秒） </p> </td> 
   <td colname="col2"> <p>ディメンション項目のシーケンスごとに、訪問者が特定のディメンション項目に関与した合計時間を表します。名前が示すように、単に「サイト」の平均に限定されるわけではありません。 シーケンスについて詳しくは、滞在時間の計算方法の節を参照してください。 </p> <p>注意：この指標は、計算での分母の違いにより、ディメンション項目レベルで訪問別滞在時間と異なる可能性が非常に高くなります。 </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p><b>廃止された指標です。</b> </p> <p>ディメンション項目の平均滞在時間が必要な場合は、「サイトでの平均滞在時間」を使用することをお勧めします。 </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>セッションの合計長さ/（起動回数 — 初回起動回数） </p> <p>Mobile アプリ SDK のみ。これは、セッションレベルの指標です。 </p> </td> 
...
<table id="table_BF1B7B8620714105BFB5C1AC37ABE02C"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>各ヒットに関する合計滞在時間（秒単位）です。ヒットレベルのディメンションであり、ページビュー数およびリンクイベントを含みます。名前が示すように、「ページ」ディメンションに限定されるわけではありません。 </p> </td> 
   <td colname="col3"> <p>Analysis Workspace </p> </td> 
  </tr> 
  <tr> 
...
> [!NOTE] ビデオビュー数や出口リ [!UICONTROL ンクなどのリ] ンクイベントがないと 、訪問の最後のヒットに費やした時間は不明です。 また、同様の理由で、[!UICONTROL バウンス訪問]（つまり、単一のヒットを含む[!UICONTROL 訪問]）には、関連付けられた[!UICONTROL 滞在時間]がありません。
...
<table id="table_D8BA825412B6420390CA78D77A5E57C2"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>「ページでの滞在時間 — 詳細」ディメンションは、ヒットレベルのディメンションです。 これを他のディメンションで分類すると、ヒットが存続して分類ディメンションも存在した秒数がわかります。 </p> <p>次の例では、検索用語「classifieds」がヒット時間54秒、59秒などに関連付けられています。これは、訪問者がその検索用語に返されたコンテンツを読み取るのに時間を費やしていることを示しています。 </p> <p><img placement="break" align="center"  src="assets/time-spent1.png" id="image_99FB62DCADDA4F8887B14333E65FF8FA" width="500px" /> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>「ページでの滞在時間 — 詳細」のディメンションに適した指標は何ですか。 </p> </td> 
...
* 「シーケンス」とは、特定の変数が同じ値を含む連続したヒットのセットです（設定によるヒット、スプレッドフォワードによるヒット、永続的なヒットのいずれか）。 例えば、prop1 "A"には2つのシーケンスがあります。ヒット1 &amp; 2およびヒット6。 最後のヒットは滞在時間を含まないので、訪問の最後のヒットの値では新しいシーケンスは開始されません。サイトでの平均滞在時間は、分母のシーケンスを使用します。

   * 滞在時間のみの目的では、ヒット2のprop1に対して上記のように、propは、ページヒットから後続のリンクヒットに「早送り」されます。 これにより、ヒット1のprop1に設定された値(「A」)が、ヒット2に費やされた時間を累積できます。
