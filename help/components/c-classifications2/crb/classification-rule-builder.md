...
* CRBがエクスポートを要求すると、分類済みと未分類の両方の値が取り込まれ、未分類の値はエクスポートの最後に取り込まれます。 つまり、時間の経過と共に、1,000 万個の分類済みの値が埋め尽くされ、未分類の値に到達しないことがあります。
