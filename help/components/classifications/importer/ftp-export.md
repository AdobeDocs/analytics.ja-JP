...
データフィルターを適用する際は、次の問題を考慮してください。

* データフィルターを定義する際に、ワイルドカードを使用できます。 アスタリスク `*` は 0 文字以上に一致し、疑問符 `?` は 1 文字に一致します。`?*` を使用すると、1 文字以上の任意の文字と一致します。
* 通常、両方のタイプのデータフィルターをダウンロードに適用すると、両方のルールに一致する行のみがダウンロードされます。 ただし、次の例外が適用されます。
   * 「空白コラムのある行」で「すべてのコラム」を選択した場合、最初のルールで指定した列以外のすべての列で、空白かどうかが確認されます。この例外により、他のすべての列も空である最初のルールに一致する列を持つ行がツールによってダウンロードされます。
   * 空の列に基づいてデータ行をダウンロードする場合は、最初のルールで指定された列を除くすべての列で空かどうかがチェックされます。
   * 両方のフィルタールールに同じ列が指定されている場合（両方の条件を満たすことはほぼ不可能）、最初のルールに一致する行のみがダウンロードされます。
...
| [!UICONTROL 受信日別にフィルター] | （任意）受信日でデータをフィルタリングします。 データをダウンロードする日付範囲を指定します。 |
| [!UICONTROL データフィルターの適用] | （任意）データ条件でデータセットをフィルタリングします。 ダウンロードをフィルタリングし、特定の値を含むデータ行や、列（分類）の値が割り当てられていないデータ行をダウンロードに含めることができます。 |
