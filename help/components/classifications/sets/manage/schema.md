...
1. 競合に関するデータの上書きを選択しますか？]****[!UICONTROL &#x200B;新しく読み込んだ分類情報で現在の分類データを上書きする場合は、以下の手順を実行します。 例：
...
>[!IMPORTANT]
>
>ファイルの読み込みと処理が正常に完了したら、クラウドの場所からファイルを手動で削除するか、ファイルのローテーションポリシーを使用して削除します。 それ以外の場合、ファイルはファイルの最終更新日に基づいて再インポートおよび再処理され、30日間のログが使用されます。 以下の例を参照してください。
>
