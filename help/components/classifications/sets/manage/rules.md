...
>title="サンプルキー"
>abstract="ルールセットをテストするためのテスト用キーを入力または貼り付けます。 各行は個別のキー値です。 「**[!UICONTROL ルールセットをテスト]**」を選択して、結果を含むダイアログを表示します。"
...
主要ディメンションの内部キャンペーンの値が`em`で始まる場合に、`Email`を&#x200B;**[!UICONTROL チャネル]**&#x200B;分類の値として割り当てるルールを定義する（例：`em:FY2025:Summer Sale`）。
...
キーディメンションの内部キャンペーンの値に`2025`が含まれている場合に、`2025`を&#x200B;**[!UICONTROL 年]**&#x200B;分類に値として割り当てるルールを定義する場合（例：`em:Summer Sale:FY2025`）。
...
`:` **[!UICONTROL トークン]**&#x200B;に基づいて、主要ディメンション内部キャンペーンの値を&#x200B;**[!UICONTROL チャネル]**、**[!UICONTROL タイプ]**、**[!UICONTROL 年]**&#x200B;の分類に分割するルールを定義します。
