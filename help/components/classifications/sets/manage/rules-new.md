...
**[!UICONTROL 次で終わる]** の値を入力します。 例：`Sale`。

#### ユースケース

キーディメンション内部キャンペーンの値に `Sale` が含まれている場合（例：**[!UICONTROL ）、]** を `Sale` タイプ `em:FY2025:Summer Sale` 分類の値として自動的に割り当てるルールを定義します。
...
**[!UICONTROL 次を含む]** の値を入力します。 例：`2025`。

#### ユースケース

キーディメンション内部キャンペーンの値が `2025` で終わる場合に、**[!UICONTROL を]** 年 `2025` 分類の値として自動的に割り当てるルールを定義します（例：`em:FY2025:Summer Sale`）。
...
キーディメンションの内部キャンペーンの値が `2025 Summer Email` と一致する場合に、**[!UICONTROL を]** タイプ `em:FY2025:Summer` 分類の値として自動的に割り当てるルールを定義します。
...
**[!UICONTROL 正規表現]** の値を入力します。 例：`^(.+)\:(.+)\:(.+)$`。

#### ユースケース

正規表現 **[!UICONTROL を適用し、キーディメンション Internal Campaign の値に一致グループ]****[!UICONTROL 、]** および **[!UICONTROL ）を使用して、]** Channel`^(.+)\:(.+)\:(.+)$`、`$1`Type および `$2`Year`$3` 分類の値を自動的に割り当てるルールを定義する必要があります。
...
* ユーザーが `Cowboys vs. Broncos` を検索する際に、`Team` タイプ **[!UICONTROL に分類]** されます。
