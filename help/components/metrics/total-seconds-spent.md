---
title: 合計滞在時間（秒）
description: ディメンション項目で費やされた合計秒数。
...
「合計滞在時間（秒）」指標は、訪問者が特定のディメンション項目に滞在した合計秒数を示します。 この指標は、特定のディメンション項目に費やした時間を生の値で計算し、他の滞在時間指標オファーのような平均値を求めない場合に役立ちます。
...
3. 2つのヒットの間隔を経過する秒数が、ディメンション項目に貢献します。
