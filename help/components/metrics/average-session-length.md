...
「Average session length (mobile)」指標は、特定のディメンション項目がディメンション項目ごとに存在する平均時間を示します。 この指標は、[サイトでの平均滞在時間](average-time-on-site.md)指標に似ていますが、この指標が計算の一部としてモバイル SDK 固有のコンポーネントを使用する点が異なります。
