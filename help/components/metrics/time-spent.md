...
**Q5: サイトでの[!UICONTROL 平均滞在時間を含む内訳の合計が親行項目と一致しないのはなぜですか]?**

A: サイトでの [!UICONTROL 平均滞在時間は、ディメンションの壊れていないシーケンスに依存するので] 、これらの実行を計算する際に、内部レポートは外部レポートに依存しません。
...
ホームページのタイムスペントを計算するときは、(30+10)/2=20となりますが、1月1日が1回のみ実行されるので、日別に分けると(30+10)/1=40となります。
