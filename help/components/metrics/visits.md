---
title: 訪問回数
description: Analytics の訪問回数指標について説明します。計算方法、影響を与える動作、その定義の変更方法などを確認します。
feature: Metrics
...
* 標準レポートスイートの場合、カスタマーケアに連絡して、特定のレポートスイートの訪問のタイムアウト（新しい訪問が開始されるヒット間の時間）を短縮するようにリクエストします。標準レポートスイートの訪問のタイムアウトは 30 分を超えないので、短縮することしかできません。

## 日付の境界をまたぐ訪問

1 回の訪問は、関係する各期間に対してカウントされます。例えば、開始が月曜日の午後 11:45 にサイトをナビゲートし、その後、火曜日の午前 12:10 に最後のイメージリクエストを送信する訪問者がある場合、月曜日と火曜日の両方の訪問と見なされます。ただし、合計訪問回数指標は重複排除され、プロジェクトの日付範囲で 1 回の訪問が表示されます。
