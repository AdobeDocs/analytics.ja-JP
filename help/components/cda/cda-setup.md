...
* 「Identified State」という新しいディメンションが使用可能です。 このディメンションは、そのヒットのExperience Cloud IDが、その時点でデバイスのグラフで知られているかどうかを判断します。
* 「ユーザー」と「個別デバイス」というラベルの付いた新しい指標を使用できます。
* 「個別訪問者」指標は「人」と「個別デバイス」に置き換えられるので、使用できません。
* セグメントを作成する際、「訪問者」セグメントコンテナは「ユーザー」コンテナに置き換えられます。

## CDA Workspaceテンプレート

アドビのオファーは、デバイス間の重要なパフォーマンスデータを確認するためのテンプレートです。
