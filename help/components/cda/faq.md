...
CDAのデバイス間のステッチは、2つの同時プロセスで行われます。

* 最初のプロセスは「ライブステッチ」と呼ばれ、データがAdobe Analyticsに流れ込むときに発生します。 ライブステッチ中にCDAが最善を尽くすのは、個人レベルでデータを再記述することです。 ただし、ライブステッチの時点でユーザーが不明な場合は、CDAが訪問者IDにフォールバックしてユーザーを表します。

* 2つ目のプロセスは「replay」と呼ばれます。 再生中、CDAは時間内に遡って過去のデータを再生し、可能であれば、指定されたルックバックウィンドウ内で履歴データを再生します。 このルックバックウィンドウは、CDAの設定要求の内容に応じて1日か7日になります。 再生中に、CDAは、ユーザーが以前に不明だったヒットをリステートしようとします。
