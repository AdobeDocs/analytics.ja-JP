...
モバイルデバイスタイプディメンションでは、[!UICONTROL フロー]ビジュアライゼーションを使用できます。
...
「[人](/help/components/metrics/people.md)」指標は、一意の個人の数をレポートするという点で「[ユニーク訪問者](/help/components/metrics/unique-visitors.md)」指標に似ています。ただし、クロスデバイス分析を使用する場合、ユニーク訪問者が CDA 以外で 2 人の別のユニーク訪問者として記録されると、ユニーク訪問者は結合されます。クロスデバイス分析が有効になっている場合は、「ユニーク訪問者」指標が「人」指標に置き換えられます。新しい指標である[一意のデバイス](/help/components/metrics/unique-devices.md)を利用できます。これは、クロスデバイス分析以外のユニーク訪問者とほぼ等しくなります。
...
デバイスグラフからフィールドベースのステッチへの切り替え、またはその逆の切り替えは、カスタマーケアからリクエストできます。 しかし、このようなスイッチを作るのに数週間かかる場合があり、以前の方法での&#x200B;*履歴的なステッチデータは失われます。*
...
## CDAに対して有効にできる会社のレポートスイートの数はいくつですか。

複数のレポートスイートを有効にすることもできますが、複数のレポートスイートが一度にリクエストされる場合、追加される各レポートスイートによって、プロビジョニングの全体的な時間が長くなります。 CDAはレポートスイートをマージしません。 CDAに対して有効になる各レポートスイートは、実際にはデバイス間で動作する必要があります（デスクトップWeb、モバイルWeb、モバイルアプリなど、複数のサーフェスからのデータを含む）。

## Experience Cloud組織（IMS組織など）の異なる地域に複数の会社が存在する場合、それらすべてに対してCDAを有効にできますか。

いいえ。同じ組織の場合、CDAを有効にできる地域は1つのみです。

## 7日間の再生と1日間の再生のそれぞれのメリットとデメリットは何ですか。

7日間の再生ルックバックウィンドウの利点は、CDAが、7日以内にログインしたユーザーに、以前に匿名イベントを関連付けようとするために、時間内にさらに遡ることができることです。 7日間のルックバックウィンドウのデメリットは、1)リプレイが1週間に1回のみ実行されること、および2)最新の7日間が変更される場合があります。

1日再生ルックバックウィンドウを使用する利点は、毎日1)再生実行、および2)昨日のみが変更される可能性があります。 1日間のルックバックウィンドウのデメリットは、CDAが1日戻って、以前に匿名イベントを昨日ログインした人と関連付けようとするのみであることです。

## 会社がAnalytics Ultimateからダウングレードすると、CDA仮想レポートスイート内のデータの関連付けが行われた場合、どうなりますか。

顧客がUltimateからダウングレードした場合、関連付けられたデータにアクセスできなくなります。 以前に関連付けられたデータはすべて削除されます。 つまり、CDA仮想レポートスイートは、デバイス間のステッチを反映しなくなります。 データは、元の未関連付けレポートスイートと似ています。
