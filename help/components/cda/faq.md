...
カスタム訪問者IDを使用することは、デバイス間でユーザーを接続するための従来の方法です。 カスタム訪問者 ID では、[`visitorID`](/help/implement/vars/config-vars/visitorid.md) 変数を使用して、訪問者ロジックに使用する ID を明示的に設定します。 `visitorID` 変数は、存在する cookie ベースの ID よりも優先されます。
