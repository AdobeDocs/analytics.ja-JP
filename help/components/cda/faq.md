...
モバイルデバイスタイプディメンションで[!UICONTROL フロー]ビジュアライゼーションを使用できます。
...
デバイスグラフからフィールドベースのステッチへの切り替え（またはその逆）は、カスタマーケアを通じてリクエストできます。 ただし、このような切り替えを行うには、完了までに数週間以上かかり、以前の方法で作成した履歴のステッチデータが失われます。**
...
## CDAに対して有効にできる会社のレポートスイートの数

複数のレポートスイートを有効にすることができますが、複数のレポートスイートが一度にリクエストされると、追加の各レポートスイートによって全体的なプロビジョニング時間が長くなります。 CDAはレポートスイートを結合しません。 CDAに対して有効な各レポートスイートは、本来、クロスデバイスである必要があります（デスクトップWeb、モバイルWeb、モバイルアプリなど、複数のサーフェスからのデータを含む）。

## Experience Cloud組織（IMS組織）に異なる地域の複数の会社がある場合、すべての会社でCDAを有効にできますか。

いいえ。同じ組織で、CDAを有効にできる地域は1つだけです。

## 7日間の再生と1日間の再生のメリットとデメリットは何ですか。

7日間の再生ルックバックウィンドウの利点は、CDAが時間通りに遡って、以前に匿名のイベントを、その7日以内にログインした一部のユーザーに関連付けようとすることです。 7日間のルックバックウィンドウのデメリットは、1)再生が週に1回だけ実行され、2)最新の7日間は変更される可能性があります。

1日の再生ルックバックウィンドウを使用する利点は、毎日1)再生実行と2)前日のみが変更される場合があります。 1日のルックバックウィンドウのデメリットは、CDAが1日戻って、以前に匿名のイベントを昨日ログインしたユーザーに関連付けようとしたときだけいうことです。

## CDA仮想レポートスイート内の関連付けられたデータをAnalytics Ultimateからダウングレードすると、どうなりますか。

顧客がUltimateからダウングレードした場合、その顧客はステッチされたデータにアクセスできなくなります。 以前にステッチされたデータはすべて削除されます。 つまり、CDA仮想レポートスイートにクロスデバイスのステッチが反映されなくなります。 データは、元の未関連付けレポートスイートと似ています。
