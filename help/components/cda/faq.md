...
上の図に示すモバイルデバイスタイプの例を使用すると、ユーザーがモバイルデバイスタイプとデスクトップデバイスタイプの間をどのように移動しているかを確認できます。 ただし、デスクトップブラウザーとモバイルブラウザーを区別することはできません。 このインサイトを得るには、デスクトップのブラウザー、モバイルブラウザーまたはモバイルアプリでエクスペリエンスが発生したかどうかを記録するカスタム変数(propやeVarなど)を作成します。 次に、前述のように、「モバイルデバイスタイプ」ディメンションの代わりにカスタム変数を使用して、フロー図を作成します。 この方法では、デバイス間の動作に関する表示が少し異なります。
...
* **フィールドベースのステッチを使用する場合**、ログインしたユーザーの識別に役立てるために選択したpropまたはeVarが、他の識別子より優先されます。共有デバイスは、同じデバイスから作成された場合でも、別のユーザーと見なされます。
...
[People](/help/components/metrics/people.md)と[個別訪問者数](/help/components/metrics/unique-visitors.md)の両方の指標は、個別訪問者（個人）をカウントすることを目的としています。 ただし、2つの異なるデバイスが同じユーザーに属する可能性を考慮します。 CDAは2つのデバイスを同じ人物にマッピングし、2つのデバイスはCDA外で2つの個別「個別訪問者」として記録されます。

## CDA の「一意のデバイス」指標と CDA 以外の「ユニーク訪問者」指標の違いを教えてください。

これら 2 つの指標は、ほぼ同じです。2つの指標の違いは、次の場合に発生します。

* 共有デバイスは複数のユーザーにマッピングされます。 このシナリオでは、1人の個別訪問者がカウントされ、複数の個別デバイスがカウントされます。
* デバイスには、同じ訪問者からの未関連付けトラフィックとステッチトラフィックの両方があります。 例えば、ブラウザーで生成された識別されたステッチトラフィックと、ステッチされなかった履歴の匿名トラフィック。 この場合、1人の個別訪問者がカウントされ、2人の個別デバイスがカウントされます。
...
## Adobeは、フィールドベースのステッチで使用されるpropまたはeVarの一意の制限をどのように処理しますか。

CDAは、識別子変数ディメンション項目がレポート用に最適化される前に取り込みます。 CDA の目的で固有の制限について心配する必要はありません。ただし、WorkspaceプロジェクトでそのpropまたはeVarを使用しようとした場合、[（低トラフィック）](/help/technotes/low-traffic.md)ディメンション項目を表示できます。
...
## ソースレポートスイートとCDA仮想レポートスイートでヒットの合計数が異なるのはなぜですか。

CDAは、複数の依存コンポーネントを持つ複雑な並列処理パイプラインを使用します。 元のレポートスイートとCDA仮想レポートスイートの間の合計ヒット数に対して、約1%のデータが一致しないことが予想されます。 クロスデバイス機能への影響は最小限です。

## 「識別された人」指標が水増しされるのはなぜですか？

識別子のprop/eVar値が[ハッシュの競合](/help/implement/validate/hash-collisions.md)に発生した場合、「識別された人」指標の数が若干増える可能性があります。

識別子prop/eVarで大文字と小文字が区別される場合、「識別された人」指標の数が大幅に増える可能性があります。 例えば、`bob`と`Bob`は同じ人物であると想定されますが、大文字と小文字の区別を使用すると、これら2つの値が区別されます。

## 識別子prop/eVarを「未識別の人」指標と共に表示すると、値が表示されるのはなぜですか。

この状況は、通常、訪問者が認証済みヒットと未認証ヒットの両方をレポートウィンドウで生成し、[再生](replay.md)がまだ実行されていない場合に発生します。 再生する前に、訪問者は[識別状態](/help/components/dimensions/identified-state.md)ディメンションの「未識別」と「識別済み」の両方に属し、一部の訪問者は、識別されていないヒットを識別子に関連付けます。 訪問者は、再生が実行されるまで（組織でのCDAの設定に応じて日単位または週単位）、この状態のままになります。 再生後のデータに対してのみレポートを実行すると、この状況が軽減されます。
