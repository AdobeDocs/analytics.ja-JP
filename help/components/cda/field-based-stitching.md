...
* ユーザーがログインしたりメールを開いたりする場合など、可能な限り個々のユーザーを一意に識別する prop または eVar を実装に設定する必要があります。この要件は、すべてのプラットフォーム（使用する場合はモバイルアプリを含む）に適用されます。propまたはeVarにデフォルト値を割り当てないでください。
