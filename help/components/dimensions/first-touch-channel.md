...
アドビのデータ収集サーバーに送信されるすべてのヒットは、レポートスイートのマーケティングチャネルの処理ルールを通じて実行されます。 各ルールを数値順に繰り返し処理し、一致が見つかると、そのマーケティングチャネルがヒットに結び付けます。 ファーストタッチチャネルは、訪問者がサイトに 30 日（デフォルトの訪問者エンゲージメント期間）を超えて訪問しない状態にならない限り、維持されます。
