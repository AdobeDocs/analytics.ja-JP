...
「リファラータイプ」ディメンションは、訪問者がクリックスルーしてサイトに到着した汎用チャネルをレポートします。Adobe maintains the rules for each dimension item, unlike [Marketing channels](marketing-channel.md), where your organization maintains rules for each channel.
...
Dimension項目には、ヒットの転送者のタイプが含まれます。 以下に具体的な変更点を示します。
...
* **「検索エンジン」ディメンション項目内の検索エンジンドメイン**:転送者は、Adobeごとに検索エンジンとして分類するすべての条件を満たしていました。 参照ドメインは有効な検索エンジンであり、*そして*、参照 URL にはキーワードクエリー文字列パラメーターが含まれています。
* **「その他のWebサイト」ディメンション項目の検索エンジンドメイン**:参照URLが検索エンジンとして分類するすべての条件を満たしていませんでした。 一般的な例としては、検索以外の他の機能専用のサブドメインがあります。例えば、`mail.google.com` または `autos.yahoo.com` は検索エンジンではなく、検索に一般的に関連付けられるトップレベルドメインに存在します。これらのサブドメインにはキーワードクエリ文字列が含まれないので、サブドメインは「検索エンジン」ではなく「その他の Web サイト」の下に含まれます。
