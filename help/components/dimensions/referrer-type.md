...
次のリストは、アドビが使用する「ソーシャルネットワーク」参照テーブルを参照しています。 アドビは、このリストをAdobe Analyticsのお客様に対して提供しています。 アドビがこのリストにドメインを追加することを推奨する場合は、組織のサポート委任者がカスタマーケアにお問い合わせください。

>[!NOTE] このリストは、マー [ケティングチャネルの処理ルールのソーシャルネットワークのデフォルトリストとは異なります](../c-marketing-channels/c-rules.md)。
