...
指定されたエントリ [ ディメンション ](overview.md)は、関連するトラフィック変数に基づいています。 Adobeは、訪問時にその変数に対して最初に見られた値から各エントリディメンションを導き出します。設定する専用の変数はありません。 入口変数以外の変数にデータが含まれている場合は、関連する入口ディメンションにもデータが含まれます。 トラフィック変数にデータが含まれる場合、入口ディメンションに対して実装の変更は必要ありません。

| プロパティ | 値 |
| --- | --- |
| **AppMeasurement変数** | なし（訪問者の最初のヒットから派生） |
| **Web SDK / XDM フィールド** | なし（訪問者の最初のヒットから派生） |
| **クエリパラメーター** | 該当なし |
| **XML タグ** | 該当なし |
| **バイト制限** | 該当なし |
| **永続性** | 訪問 |
...
「入口ページ（オリジナル）」ディメンションの動作は、他の入口ディメンションとは異なります。 特定の訪問に対する入口ページを保持するのではなく、その訪問者の cookie の有効期間全体にわたって、最初の入口ページを保持します。 例えば、サイトへの初回訪問で `https://example.com/page4` を訪問して、1 年後に `https://example.com/store` を訪問した場合は、「オリジナルの入口ページ」はディメンション項目として `https://example.com/page4` をリストします。
