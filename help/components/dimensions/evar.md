...
レポートで使用される（大文字と小文字の）区別は、特定のカレンダー月に送信する最初の値に基づきます。 この表記は、レポートウィンドウと、その期間中に最初に収集された eVar 値の表記に応じて変わる場合があります。

## eVar にデータを入力する

実装時にeVarを明示的に設定します。 値は保持され、割り当てと有効期限に基づいて指標に関連付けられます。以下の[eVarsの仕組み](#how-evars-work)を参照してください。

| プロパティ | 値 |
| --- | --- |
| **AppMeasurement変数** | [`eVar1` - `eVar250`](/help/implement/vars/page-vars/evar.md) |
| **Web SDK / XDM フィールド** | [`xdm._experience.analytics.customDimensions.eVars.eVar1` - `eVar250`](https://experienceleague.adobe.com/ja/docs/experience-platform/xdm/field-groups/event/analytics-full-extension) |
| **クエリパラメーター** | [`v1` - `v250`](https://developer.adobe.com/analytics-collection-apis/methods/data-insertion/variable-reference#variables) |
| **XML タグ** | [`<eVar1>` - `<eVar250>`](https://developer.adobe.com/analytics-collection-apis/methods/data-insertion/variable-reference#variables) |
| **バイト制限** | 255 バイト |
| **永続性** | 設定可能 |
...
成功イベントと eVar は、異なるタイミングで定義されることがよくあります。 この `post_evar` 列では、eVar 値をイベントに結び付け、レポート内のデータを表示できます。 次の訪問を例に説明します。

1. 訪問者が自社サイトのホームページに到達します。
2. ユーザーはサイトの内部検索で「cats」を検索します。 実装では、内部検索に eVar1 を使用します。
3. 訪問者は製品を表示し、チェックアウトプロセスに進みます。
...
Analysis Workspace などの Adobe Analytics のツールは、この収集されたデータを基に動作します。 例えば、ディメンションに eVar1、指標に注文を使用してレポートを取得すると、次のようなレポートが表示されます。
...
配分と有効期限は永続的な値を決定するので、Analytics の実装から最大限の価値を引き出すのに不可欠です。 各 eVar に対する複数の値の処理方法（配分）や、eVar がデータの保持をいつ停止するか（有効期限）について、社内で話し合うことを強くお勧めします。

* デフォルトでは、eVar は最後に設定された値にクレジットを配分します。 新しい値を指定すると、永続値が上書きされます。
* デフォルトでは、eVar の有効期限は「訪問終了時」に設定されています。 訪問が終了すると、値は `post_evar` 列の行間でコピーされなくなります。

eVar の配分と有効期限は、レポートスイート設定の「[コンバージョン変数](/help/admin/tools/manage-rs/edit-settings/conversion-var-admin/conversion-var-admin.md)」で変更できます。

## Prop に対する eVar の値

アドビでは、ほとんどの場合に eVar を使用することを推奨しており、その理由は次のとおりです。
