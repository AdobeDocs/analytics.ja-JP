...
>[!IMPORTANT]
>
>プライバシー保護の取り組みが進んでいるため、ほとんどの検索エンジンでは検索キーワードが渡されなくなりました。 アドビが検索エンジンを認識しても、ディメンション項目 `"Keyword unavailable"` の下にキーワードグループがないヒット。
...
Adobeは、各ヒットの検索エンジン [referrer](referrer.md)からこのディメンションを取得し、リファラーのクエリ文字列からキーワードを抽出します。 設定する変数がありません。 各値はリファラーに依存するため、リファラーディメンションと[内部URL フィルター](/help/admin/tools/manage-rs/edit-settings/general/internal-url-filter-admin.md)が正しく設定されていることを確認してください。
