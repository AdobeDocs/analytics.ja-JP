...
* **ページでの滞在時間 — グループ**: 時間がグループ化されます。 ディメンション項目の範囲はから `"Less than 15 seconds"` になり `"More than 30 minutes"`ます。 ページ表示間の時間は通常30分を超えません。 ただし、タイムスタンプ付きのヒットまたはデータソースを使用する場合、ページ表示間の時間は30分を超える可能性があります。
