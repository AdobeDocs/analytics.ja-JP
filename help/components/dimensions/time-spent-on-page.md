...
* **ページでの滞在時間 — グループ**：時間がグループ化されます。Dimension items range from `"Less than 15 seconds"` to `"More than 30 minutes"`. ページビュー間の時間は通常 30 分を超えません。 ただし、タイムスタンプ付きのヒットまたはデータソースを使用する場合、ページビュー間の時間は 30 分を超える可能性があります。
