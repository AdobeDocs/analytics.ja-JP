...
「概要データソース」により、集計された指標を Adobe Analytics にレポートスイートレベルでインポートできます。概要データソースのアップロードには、訪問者ID *を持たない集計指標が含まれるので、[!UICONTROL 訪問]コンテナおよび[!UICONTROL 訪問者]コンテナではセグメント化できません。* VRSはセグメント化を使用して動作するので、サマリデータソースを使用してインポートしたデータは、仮想レポートスイートでは訪問コンテナまたは訪問者コンテナを使用して作成されたセグメントを使用できません。
