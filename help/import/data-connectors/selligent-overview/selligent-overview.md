...
| Recipient ID | 統合では、「訪問者ID」を取得し、Analytics変数(eVar)内に保存する必要があります。 訪問者ID（「受信者ID」とも呼ばれる）は、Selligentシステムからの電子メールアドレスをエンコードまたは数値で表したものです。 この「受信者ID」は、サイト上でのダウンストリーム訪問者の行動（買い物かごの放棄、購入など）に関連付けられます。Selligentシステムに取り込まれ、再マーケティングに利用できます。 セットアッププロセスの一環として、ウィザードの指示に従って、この目的のeVarを指定する必要があります。 |
