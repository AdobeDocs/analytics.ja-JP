...
この手順では、クリック課金型データの1回限りの読み込みを設定する方法について説明します。 ただし、[!UICONTROL データソース]では、ここで説明するファイル形式を使用してデータを継続的にインポートできます。 使用する有料検索プラットフォームによっては、定期的なエクスポート（日次、月次など）をスケジュールしたり、エクスポートを Adobe Analytics に必要なファイル形式に変換するための自動プロセスを設定したり、これらのファイルを Adobe Analytics にアップロードして、有料検索の継続的な統合レポーティングに利用したりできる場合があります。
...
エクスポートに、日付、宛先 URL（ランディングページ）、インプレッション、クリック数、コストのフィールドが含まれていることを確認します。 エクスポートにはその他のフィールドを含めることができますが、次の手順の中で削除します。
1. 可能であれば、レポートを `.csv` またはタブ区切りファイルとして保存します。 これにより、次の手順での作業が簡単になります。
1. ファイルを Microsoft Excel で開きます。

### Microsoft Excel でファイルを編集する
...
e. ウィザードの手順3で、データをプレビューし、列の1つが「trackingcodename=trackingcode」であることを確認します。 追加の変数がある場合は、&amp; を区切りとして使用して、これらの手順を繰り返します。
...
### FTP を使用してファイルを Adobe Analytics にアップロードする
